...
export function queryPageList(query) { // 获取代课信息列表
  return fetch({
    url: '/api/course/dk/queryPageList',
    method: 'post',
    data: query
  });
}
...
export function clslbz(query) {
  return fetch({
    url: '/api/course/dk/clspbz',
    method: 'post',
    data: query
  });
}

export function getDataInfo(query) { // 是否有模板
  return fetch({
    url: '/api/course/dk/selectDkDetail',
    method: 'get',
    params: query
  });
}
