    ...
    methods: {
      ...
      clspbz(allbz) {
        ...
        if (slzt == this.slztEnum.SPZ) {
          ...
          zsbz = _.filter(allbz, obj => obj.spzt == this.bzztEnum.SPWC || obj.spzt == this.bzztEnum.HQWC);
        }
        const zsslbz = [];
      ...
      showYwz(bdx) {
        ...
        try {
          if (bdx.bdxlx === 5) {
            ...
            if (bdx.bdxmc.indexOf('明细') === -1) {
              showHtml = '<span>' + bdx.ywz + '</span>'
            } else {
              const arry = bdx.ywz.split(',');
              const sb = [];
              arry.forEach(e => {
                if (e.length > 0) {
                  sb.push('<span>' + e + '</span><br>');
                }
              })
              showHtml = sb.join('');
            }
