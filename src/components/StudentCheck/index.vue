...
import {getStudentByClassCode} from '@/api/admin/student/index';
	...
	methods: {
        ...
        async getGroupStudent(){
            if(this.groupStudent.length>0){
                ...
                var res = await getBjTree(localStorage.getItem('xxdm'));
                this.groupStudent = res;
                this.groupList = res;
            }
        },
        async getCurrentXnxq(){
            var res = await getCurrentXnxq({"xxdm":localStorage.getItem('xxdm')});
        ...
        async getGroupInfoList(nodeData){
            ...
            if(nodeData.bjdm){
                ...
                var res = await getStudentByClassCode({"schoolCode":localStorage.getItem('xxdm'),"classCode":nodeData.bjdm});
