...
const user = {
  ...
  actions: {
        ...
        loginByEmail(username, userInfo.password, userInfo.othername).then(response => {
          ...
          if (data.status === 500) {
            ...
            if(typeof data.data == 'string'){
              console.log(123)
              localStorage.setItem('moreUserRoleInfo', '')
              setToken(data.data);
              commit('SET_TOKEN', data.data);
            }else{
              localStorage.setItem('moreUserRoleInfo', JSON.stringify(data.data))
            }
    ...
    loginByEmailadmin({
      ...
      commit('SET_ELEMENTS', undefined);
      removeToken();
      const platform = 1;       //1.为pc端登录 2.综合素质移动端  3.电子班牌  4.exe版本
      return new Promise((resolve, reject) => {
        loginByEmailadmin(username, userInfo.password, userInfo.userId, kaptcha,platform).then(response => {
          const data = response;
    ...
    GetInfo({
      ...
      // console.log(state.token,"state.state")
      return new Promise((resolve, reject) => {
