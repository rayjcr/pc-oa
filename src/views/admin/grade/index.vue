  ...
  <div class="app-container calendar-list-container njApp">
    ...
    <div class="u_TableMain">
      <div class="filter-container">
        ...
        <el-button class="filter-item" style="margin-left: 10px;" type="primary" v-waves icon="search" @click="handleFilter">搜索</el-button>
        <el-button class="filter-item" style="margin-left: 10px;" @click="handleCreate('form')" type="primary" icon="edit">添加
        </el-button>
        <el-button class="filter-item" @click="njsjHandler" type="primary" icon="edit">年级升级
        </el-button>
      </div>
      <div class="m_GenTableBox">
      <el-table class="tableBorder" :key='tableKey' :data="list" v-loading.body="listLoading"  fit highlight-current-row
                style="width: 100%">
        <el-table-column align="center" width="200" label="年级代码">
        ...
        <el-table-column align="center" width="250" label="年级类型">
          <template slot-scope="scope">
        ...
        <el-table-column align="center" label="操作" width="300">
          <template slot-scope="scope">
            <el-button size="small" type="success" @click="setGradeManager(scope.row)">设置年级主任</el-button>
    ...
    <el-dialog class="njsjDialog" :close-on-click-modal='false' title="年级升级" width='450px' :visible.sync="dialogTableVisible">
      <div>
        前一学年：
    ...
    <el-dialog :title="textMap[dialogStatus]" :close-on-click-modal='false' :visible.sync="dialogFormVisible" width="600px">
	...
	<multipleselect ref="selectTeacher" v-bind:teacherList="teacherList" v-bind:checkTeacher="checkteacher" v-bind:multipleLimit='5' @handleChangeSel="handleChangeSel"></multipleselect>
  </div>
</template>
<script>
  import {
  ...
  import { teachclassAll } from 'api/admin/teachclass/index';
  export default {
    name: 'grade',
    components: {
        multipleselect,
    ...
    methods: {
      ...
      // 设置年级主任
      async setGradeManager(row){
		  	this.curGrade = row;
			await this.getTeacherList();
			var res = await getGradeLeader({"schoolCode":localStorage.getItem("xxdm"),"gradeCode":row.njdm});
			this.checkteacher = res.value;
			for(var i=0;i<this.checkteacher.length;i++){
				this.checkteacher[i].gh = this.checkteacher[i].userId;
				this.checkteacher[i].xm = this.checkteacher[i].name;
			}
      ...
      async getTeacherList(){
          var res = await teachclassAll({xxdm:localStorage.getItem("xxdm")});
          this.teacherList = res.data.jsList;
      },
      async handleChangeSel(selTeacher){
        var param = {};
        param.gradeCode = this.curGrade.njdm;
        param.gradeName = this.curGrade.njmc;
        param.schoolCode = localStorage.getItem("xxdm");
        param.teacherList = [];
        for(var i=0;i<selTeacher.length;i++){
          param.teacherList.push({"name":selTeacher[i].xm,"userId":selTeacher[i].gh});
        }
        if(selTeacher.length==0){
          var res = await clearGradeLeader(param);
        }else{
          var res = await savaGradeLeader(param);
        }
