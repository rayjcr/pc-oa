<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-input @keyup.enter.native="handleFilter" style="width: 200px;" class="filter-item" placeholder="操作名称" v-model="listQuery.name"> </el-input>
  ...
  <span>
    ...
    <el-table-column width="200px" align="center" label="操作名称"><template slot-scope="scope">
  <span>
    {{scope.row.name}}</span>
</template>

</el-table-column>
    <el-table-column width="400px" align="center" label="操作路径"><template slot-scope="scope">
  <span>
    {{scope.row.uri}}</span>
</template>
</el-table-column>
    <el-table-column width="150px" align="center" label="请求类型"><template slot-scope="scope">
  <span>
    {{scope.row.method}}</span>
      ...
      <el-table-column  align="center" label="是否为查询操作"><template slot-scope="scope">
  <span>
    {{(scope.row.isQuery=='0')?'是':'否'}}</span>
      </template>
  ...
  <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
    <el-form :model="form" :rules="rules" ref="form" label-width="130px">
      ...
      <el-form-item label="操作名称" prop="name">
        <el-input v-model="form.name" placeholder="请输入操作名称"></el-input>
      </el-form-item>
      <el-form-item label="操作路径" prop="uri">
        <el-input v-model="form.uri" placeholder="请输入操作路径"></el-input>
      </el-form-item>
      <el-form-item label="请求类型" prop="method">
        <el-select class="filter-item" v-model="form.method" placeholder="请选择请求类型">
          <el-option v-for="item in  methodOptions" :key="item" :label="item" :value="item"> </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="是否为查询操作" prop="isQuery">
        <el-select class="filter-item" v-model="form.isQuery" placeholder="请选择是否为查询操作">
          <el-option v-for="item in  isQueryOptions" :key="item.code" :label="item.name" :value="item.code"> </el-option>
        </el-select>
      </el-form-item>
    </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      isQueryOptions: [{ code:'0', name:'是'},{code:'1',name:'否'}],
      form: {
        code: undefined,
        type: undefined,
        name: undefined,
