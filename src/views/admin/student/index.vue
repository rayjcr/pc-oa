<template>
    <div class="app-container">
        ...
        <el-row class="studentManaerwrapper">
            <div class="studentManaerleft" :class="{'hideLeft':leftHide}">
               <i class="minLeft" :class="{'el-icon-minus':!leftHide,'el-icon-plus':leftHide}" @click="hideLeft"></i>
            ...
            <div class="studentManaerright" :span="18">
                <div class="app-container calendar-list-container studentpagestyle">
                    <div class="studentpagestyletopdiv flexa">
                        <div class="studentpagestyletopbutton">
                            ...
                            <el-button class="filter-item marginleft10" @click="downloadQr" type="primary" icon="edit">导出学生二维码</el-button>
        ...
        <el-dialog title="导出二维码" :visible.sync="QrVisible" :close-on-click-modal="false" width="550px"  class="import_box">
            <el-form :model="wdform" ref="form" label-width="120px" class="collectForm">
            <el-row>
                <el-form-item required label="选择年级班级：" v-if="temarray.length==0">
                    <el-cascader class="selClass" style="width:350px;max-height: 200px;overflow: auto;" v-model="targetClass" size="small" placeholder="请选择年级班级" :options="GradeClassList" filterable :props="{multiple:true, children:'bjList', expandTrigger: 'hover' }" clearable></el-cascader>
                </el-form-item>
                <el-form-item label="尺寸">
                    <el-radio v-model="qrSize" label="100">100*100</el-radio>
                    <el-radio v-model="qrSize" label="300">300*300</el-radio>
                </el-form-item>
                <el-form-item label="二维码水印">
                    <el-upload
                        ref="upload"
                        class="avatar-uploader"
                        action="/api/base/jcXsjbsj/getStudentQrCodeListWithLogo"
                        :headers="headers"
                        :data="qrParam"
                        ...
                        <img v-if="qrImage" :src="qrImage" class="avatar">
            ...
            <div slot="footer" class="dialog-footer">
                <el-button size="medium" type="primary" :loading="isLoad" @click="downloadQrForClass">下载</el-button>
                <el-button size="medium" @click="QrVisible=false;targetClass=[]">取 消</el-button>
            </div>
        </el-dialog>
...
import {
  ...
  delObj,
  fuzzyQuery,
  getStudentQrCodeList,
  getStudentQrCodeListWithLogo
  ...
  methods: {
    ...
    */
    ...
    importSubmit(fileName) {
      ...
      //console.log(this.$refs[fileName],773)
    ...
    //下载学生二维码
    async downloadQr(){
        console.log(this.logoFile,119900)

        // if(this.temarray.length>0){
        //     var param = {};
        //     param.xxdm = localStorage.getItem("xxdm");
        //     param.dqxn = this.dqxn;
        //     param.studentList = this.temarray.join(",");
            

        //     // console.log(param,1137)
        //     var response = await getStudentQrCodeList(param);
        //     // var response = await getStudentQrCodeListWithLogo(param);

        //     const blob = new Blob([response], {type:'application/octet-stream;charset=utf-8'});
        //     const downloadElement = document.createElement('a');
        // 　　const href = window.URL.createObjectURL(blob); // 创建下载的链接
        // 　　downloadElement.href = href;
        // 　　downloadElement.download = '学生列表.zip'; // 下载后文件名
        // 　　document.body.appendChild(downloadElement);
        // 　　downloadElement.click(); // 点击下载
        // 　　document.body.removeChild(downloadElement); // 下载完成移除元素
        // 　　window.URL.revokeObjectURL(href); // 释放掉blob对象
    ...
    beforeAvatarUpload(file){
        ...
        if (!isJPG) {
            this.$notify({title: '错误',message: '上传头像图片只能是 JPG 格式!',type: 'warning',duration: 1000})
        }
        if (!isLt2M) {
            this.$notify({title: '错误',message: '上传头像图片大小不能超过 2MB!',type: 'warning',duration: 1000})
        }
        return isJPG && isLt2M;
    },
    ...
    handleChange(file){
        ...
        reader.onload = function (e) { 
            console.log(e)
			that.qrImage = this.result; 
			// document.getElementById("data").innerText=this.result.substring(this.result.indexOf(',')+1); 
		}
        console.log(this.logoFile,1250);
    },

    

    async downloadQrForClass(){
        this.isLoad = true;
        var formData = new FormData();
        formData.append("xxdm",localStorage.getItem("xxdm"));
        formData.append("dqxn",this.dqxn);
        formData.append("size",parseInt(this.qrSize));
        var param = {};
        param.xxdm = localStorage.getItem("xxdm")
        param.dqxn = this.dqxn
        param.size = parseInt(this.qrSize)

        if(this.temarray.length==0){
            if(this.targetClass==[]||this.targetClass.length==0){
                this.$notify({title: '错误',message: '请选择班级',type: 'error',duration: 1000})
                this.isLoad = false;
                return
            }  
            var classListArr = [];
            for(var i=0;i<this.targetClass.length;i++){
                classListArr.push(this.targetClass[i][1]);
            }
            formData.append("classList",classListArr.join(","));
            param.classList = classListArr.join(",");
        }else{
            formData.append("studentList",this.temarray.join(","));
            param.studentList = this.temarray.join(",");
        }

        if(Object.keys(this.logoFile).length!=0){
            formData.append("file",this.logoFile);
            try{
                var response = await getStudentQrCodeListWithLogo(formData);
                this.isLoad = false;
                const blob = new Blob([response], {type:'application/octet-stream;charset=utf-8'});
                const downloadElement = document.createElement('a');
            　　const href = window.URL.createObjectURL(blob); // 创建下载的链接
    ...
    async getAllGradeClass(){
        var res = await baseClassApi.GetNjBjList({xxdm:localStorage.getItem("xxdm"),xndm:this.dqxn});
...
.studentpagestyletopdiv {
    width: 100%;
    float: left;
    padding: 15px 0;
}
.studentpagestyletopsearch {
    float: right;
    text-align: right;
    padding: 5px 0;
}
...
.selClass /deep/ .el-cascader__tags{
  top: 5px;
