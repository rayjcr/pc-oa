...
export default {
  ...
  methods: {
    ...
    async getUserActiveF() {
      ...
      await getGateLog(this.listQuery).then(response => {
        this.activeties = response.data.rows;
        if (this.activeties != null && this.activeties.length > 0) {
          for (const o of this.activeties) {
    ...
    open() {
      this.$router.push({ path: '/userDynamic' });
    }
  }
};
</script>
