...
export default {
  ...
  data() {
    return {
        isopen: false, // 展开收起判定条件
        rankd: false, // 是否按人次段分配
        options: [{
          value: 1,
          label: 1
        }, {
        ...
        checkList:[], // 勾选分班依据数据项
        bjnum:0,
        studentclass: { // 获取学生人数参数
            SchoolCode: localStorage.getItem('xxdm'),
            Batch: '' // 批次
        },
        studentparams: { // 学生数据集合
            schoolCode: '', // 学校代码
            batch: '', // 批次
            classNum: 0, // 班级数
            averageScoreDifference: 0.0, // 
            scoreAverageScoreDifference: 0.0, // 平均分分配参数
        ...
        newRank: [], // 新名次段
        oldRank: [], // 久名次段
        copyranklist: [], // 名次段备份数据
        rankHasStudentnum: [], // 名次段学生数（名次段长度）
        rankSasRankNo: [], // 
        surestudentnum: false, // 是否可分班判定条件
        changerank: false, // 改变名次段判定条件
        rankorNo: [], // 名次段排序数据
        ispadden: false, // 数据去重判定条件
        fenbanyouxianji: [],// 分班优先级数据
        basis:[
            {type:0,orderNo:1},
            {type:1,orderNo:2},
            {type:2,orderNo:3}
  ...
  mounted () {
    ...
    this.batchs = JSON.stringify(GetYear()) // 获取当前批次
  ...
  methods: {
    ...
    addcondition () {
        if (!this.Rank.length) {
            ...
            this.Rank[0].orderNo = this.num+1 // 添加排序依据
    ...
    repeatCode (arr, personnum, strs, personstrs, stu, ran) {
        ...
        this.isRepeat(this.rankorNo) // 验证重复数据
                ...
                // } else {
                    ...
                    if (!this.ispadden) {
                        if (this.Rank.length >= this.Ranklength) {
                            ...
                            if (this.Ranklength !== 0 && this.newRank.length) { // 根据需求对名次段进行修改及添加
                                this.changerank = true
                                this.editrange(this.oldRank) // 修改名次段
                                this.postrank(this.newRank)  // 添加名次段
                    ...
                    if (fractions.test(this.studentparams.averageScoreDifference) && fractions.test(this.studentparams.scoreAverageScoreDifference)) {
                        ...
                        // } else {
                            if (alloationAccrdingRanking) {
                                ...
                                if (sexAverageEmpoty) { // 是否性别均分数据判定
                                    // if (emptyOrderNoAlloationAccrding) {
                                    //     this.$notify({title: '提示', message: '优先级不能为空', type: 'error', duration: 2000})
                                    //     this.isloading = false
                                    // } else {
                    ...
                    if (alloationAccrdingRanking) {
                        ...
                        if (sexAverageEmpoty) {
                            ...
                            // } else {
                                this.studentparams.averageScoreDifference = 0
                                this.studentparams.scoreAverageScoreDifference = 0
                                this.autoAllocationStudent(this.studentparams) // 分班调用
                            // }
                        } else {
                            this.studentparams.averageScoreDifference = 0
                            this.studentparams.scoreAverageScoreDifference = 0
