...
export default {
    ...
    data() {
        return {
            ...
            multipleSelection1:[], // 第一个班级查看班级数据
            multipleSelection2:[], // 第二个班级查看班级数据
            class1id: '', // 第一个班id
            class2id: '', // 第二个班id
            students1id: [], // 第一个班级学生id集合
            students2id: [], // 第二个班级学生id集合
            params1: [], // 交换班级学生参数
            params2: [], // 交换班级学生参数
            rankarr: [], // 名次段数据列表
            singledifference: [], // 平均分极差数据
            rankingCountDifferences: [], // 名次段人数极差数据
            othercourselist: [], // 课程数据
            studentothercourselist: [], // 学生课程数据
            from: '', // 进入页面的路径
            routerbatch: '' // 批次
        }
    },
    mounted: function(){
        ...
        //     })
        // })
        this.from = this.$route.query.from ? this.$route.query.from : '' // 获取进入页面路径
        this.routerbatch = this.$route.query.datas ? this.$route.query.datas : JSON.stringify(GetYear()) // 获取批次
        let params = {
        ...
        getcalsslist (params) {
            ...
            PostClassList(params).then(data => {
                ...
                for (var i=0;i<this.bjList.length;i++) {
                    this.bjList[i].rankingCounts.forEach(item => { // 名次段数据处理
                        this.rankarr.push({count: item.count, name: item.name, rankingId: item.rankingId, type: item.type})
                    })
                    this.bjList[i].courseAverageScores.forEach(item => { // 课程数据处理
