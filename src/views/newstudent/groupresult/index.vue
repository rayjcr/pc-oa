...
export default {
    ...
    data() {
        return {
            tabletitle: '', // 表格头信息
            isable: true, // 输入框是否可编辑
            isedit: false, // 是否编辑显示控制
            tableData: [], // 表格数据项
            ranklist: [], // 名次段数据列表
            input: '', // 
            dialogRankVisible: false, // 名次段编辑弹层控制
            dialogModelVisible: false, // 通知说明打印弹层显隐控制
            studentinfo: [], // 学生列表信息
            title: ['班级编号', '班级人数', '男生数', '女生数', '男女比例', '成绩平均分'],
            expands: [],
            show: false, // 打印弹窗内折叠栏控制
            ischose: false, // 打印弹层内模拟输入框是否获取焦点
            msg: '', // 模拟输入框输入信息
            dialogFormVisible:false, // 打印预览弹层显隐控制
            bjinfo: { // 班级数据信息
                classInfo: []
            },
            allData:[],
            checkBj: '',
            textarea2:'', // 入学说明数据绑定数据
            xxdm: localStorage.getItem('xxdm'), // 学校代码
            batchs: '', // 批次
            classlist: [], // 班级列表
            paramscanshu: {}, // 获取班级信息参数
            classintroduce: {}, // 入学说明数据
            schoolCode: '', // 学校代码
            batch: '', // 批次
            Listoflist: [],
            firstfewlist: [], // 初次获取名次段数据列表
            iskong: false, // 是否为空（用于编辑判定）
            islength: false, // 是否具有长度（用于统计判定）
            isnegative: false,// 用于编辑判定
            ranklistlength: 0, // 名次段数据长度
            newranklist: [], // 新名次段数据
            oldranklist: [], // 原名次段数据
            ischangerank: false, // 是否修改名次段
    ...
    mounted: function(){
        ...
        if (this.$route.query.from === 'history') {
            ...
            this.getclasslist(params) // 获取班级列表
            ...
            GetGroupHistory({xxdm: localStorage.getItem('xxdm')}).then(data => { // 获取历史纪录
    ...
    methods: {
        ...
        startstastic () {
            ...
            if (this.ranklist.length) { // 名次段数据条件判定
                this.islength = true
