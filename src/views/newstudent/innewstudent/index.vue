...
export default {
  ...
  data() {
    return {
      ...
      schoolCode: '', // 学校代码
      batch: '', // 批次
      callbackStatus: [], // 上传后返会数据列表
      dialogDLDetailVisible: false, // 导入学生返回数据查看弹层
      value: '', // 条件筛选绑定值
      options:[ // 下拉选框列表
        {value: null, label: '全部'},
        {value: 1, label: '男生'},
      ...
      studentNumber: {}, // 学生人数数据
      ready: false, // 在线导入是否准备好判定
      dialogDLFormVisible: false, // 删除学生弹层控制
      studentId: [], // 学生id列表
      ischose: false, 
      msg: '',
      show: false,
      courseselected: {},
      studentpage: { // 分页获取学生参数
        keywords: '', // 搜索关键字
        sex: null, // 性别
        schoolCode: localStorage.getItem('xxdm'), // 学校代码
        batch: '', // 批次
        pageIndex: 1, // 页码
        pageSize: 10 // 页面数据长度
      },
      allData:[],
      dialogFormVisible: false, // 添加学生信息弹层控制
      dialogStatus: '', // 判定数据是创建还是修改
      sexOptions: [{sex: '女生', value: 2},{sex: '男生', value: 1}], // 创建新生性别控制单选控制
      form:{ // 创建新生表单数据参数
        name:'',
        sex:'',
        idCard:'',
      ...
      courselist: [], // 课程列表
      // 学生导入
      importXSFormVisible: false, // 导入弹层控制
      fileList: [], // excel上传文件流数据
      importLoading: false, // 导入加载状态
      datas: { //导入数据
        Batch: '',
        SchoolCode: ''
      },
      studentClass: {}, // 获取新生数量参数
      isadd: false, // 是否新增学生
      isidentical: false,
      ispadden: false, // 数据重复判定条件
      ischangeidentical: false,
      ischanged: false, // 是否为更改
      newcourselist: [], // 新的课程列表
      addcourselist: [], // 原课程列表
      sourcescore: false // 课程数据源
    }
  },
  computed: {
    headers () {
  ...
  mounted: function(){
    ...
    this.getstudentnum(this.studentClass) // 获取学生数量
    this.studentpage.batch = this.$store.state.division.batch
    this.allData = tempdata;
    Getxndm({xxdm: this.batch}).then(data => { // 获取学年代码
      let params = {
        page: 1,
        limit: 30,
    ...
    this.getstudentlist(this.studentpage) // 获取学生列表
    // this.resetData();
  },
  methods: {
    // 表格筛选功能
    ...
    changestudent (formName) {
      ...
      if (this.form.scores.length) { // 如果数据存在未修改否则为新增 （修改）
    ...
    addstudent (addobj, formName, str) { // 规则验证并确认添加或不予添加
      var arr = []
      var strs = ''
      var space = /(^\s+)|(\s+$)/g;
      var myreg = /^[1][3,4,5,7,8][0-9]{9}$/;
    ...
    sureaddstudent (form, formName) { // 确认添加新生
