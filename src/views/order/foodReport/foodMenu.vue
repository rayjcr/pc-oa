        ...
        <div class="u_TableMain columns ofhide">
            <div class="filter-container innerTab">
                <el-form :inline="true" :model="searchFrom" class="demo-form-inline">
                    ...
                    <el-form-item>
                    <el-button type="primary" icon="el-icon-download" @click="handleDownload" :loading="downloadLoading" class="handle-del mr10">导出</el-button>
                    </el-form-item>
                </el-form>
            </div>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      api.getMenuPageCountList(this.searchFrom).then(res => {
    ...
    /** 下载excel**/
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['菜品名称', '预计用餐数量', '实际用餐数量']
        const filterVal = ['name', 'planNum', 'actualNum']
        const list = this.tableData
        const data = this.formatJson(filterVal, list)
        excel.export_json_to_excel(tHeader, data, '菜品统计表格')
