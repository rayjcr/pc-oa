        ...
        <div class="u_TableMain columns ofhide">
            <div class="filter-container innerTab">
                <el-form :inline="true" :model="searchFrom" class="demo-form-inline">
                    ...
                    <el-form-item>
                    <el-button @click="handleDownload" :loading="downloadLoading" type="primary" icon="el-icon-download" class="handle-del mr10">导出</el-button>
                    </el-form-item>
                </el-form>
            </div>
...
export default {
    ...
    getList() {
        this.loading = true
        api.getUserPageCountList(this.searchFrom).then(res => {
        if (res.resultCode === 1) {
            ...
            this.loading = false;
            this.$notify({ title: '失败', message: res.resultMessage, type: 'warning', duration: 2000 });
        }
        });
    },
    ...
    handleDownload() {
        ...
        const tHeader = ['人员名称', '人员角色', '认证次数', '用餐次数']
        const filterVal = ['name', 'role', 'authCount', 'dinCount']
        const list = this.tableData
        const data = this.formatJson(filterVal, list)
        excel.export_json_to_excel(tHeader, data, '用户点餐统计表格')
