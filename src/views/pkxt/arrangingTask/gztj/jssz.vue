  ...
  <div class="jssz">
    ...
    <el-row class="mt20">
      <div class="pkleftmenu" v-show="setTc!='合班设置'">
        <div class="tit" v-show="setTc != '固定排/不排设置'">选择排课教师</div>
        <div class="tab-box" v-show="setTc == '固定排/不排设置'">
          <div class="tab-item" :class="{'active':choiceType == 'one'}" @click="setPkjs('one')">单人设置</div>
          <div class="tab-item" :class="{'active':choiceType == 'batch'}" @click="setPkjs('batch')">批量设置</div>
        </div>
        <div class="tcfilter">
          <el-input size="small"
      ...
      <div v-show="setTc=='固定排/不排设置'" class="pkrightmenu">
        <el-card shadow="never" v-show="choiceType == 'one'">
        ...
        <el-card shadow="never" style="margin-bottom: 5px;" v-show="choiceType == 'one'">
          <div slot="header" class="clearfix">
            <span>设置过的课程</span>
          </div>
          <div>
        ...
        <!--批量设置新增组-->
        <el-card shadow="never" v-show="choiceType == 'batch'">
          <div slot="header" class="clearfix">
            <span>设置老师组</span>
          </div>
          <div class="js-group-box">
            <ul>
              <li class="flex alignCenter" v-for="(item,index) in groupJsList" :key="index">
                <div class="group-key">第{{index+1}}组</div>
                <div class="flex flex_1 js-list flexWrap">
                  <div class="js-item" v-if="item.jsList.length" v-for="(c,i) in item.jsList" :key="i">
                    {{c.name}}
                    <span class="remove-js" v-if="c.name && item.showCheckboxs"
                          @click="delJsFromGroup(index,i)">×</span>
                  </div>
                </div>
                <div class="js-btn-box">
                  <el-button v-if="!item.showCheckboxs" @click="batchSetJs(index,'set')" type="primary" plain>设置老师
                  </el-button>
                  <el-button v-else @click="batchSetJs(index,'complete')" type="primary" plain>完成</el-button>
                  <el-button @click="getTearchGrouyBycodes(index)" type="primary">设置</el-button>
                  <div class="del-group-btn" v-if="index != 0" @click="delGroup(index)"><span>×</span>删除组</div>
                </div>
              </li>
            </ul>
...
<script>
  import {
    ...
    getJspk,
    getTearchGrouyList,
    getTearchGrouyBycode,
    saveBatchTearchCourse,
    getUpdateTearch,
    deleteGroupTeacher
  ...
  export default {
    ...
    data() {
      return {
        choiceType: 'one', // one,单人设置，more,批量设置
        showCheckboxs: false, // 树形是否显示复选框
        groupJsList: [{showCheckboxs: false, jsList: []}], // 存放复选框选中的教师组
        checkedKeys: [], // 树形复选框默认选中项
        groupKey: 0,
        count: 1,
        lskc: '',
        lsjs: '',
    ...
    methods: {
      ...
      lookClassDetail(item, index) {
        this.num = index;
        this.kcdm = this.kcdmtemp = item.kcdm;
      ...
      batchSetJs(key, type) {
        this.loadTree();
        this.groupJsList.forEach((c, i) => {
          if (i == key) {
            this.showCheckboxs = this.groupJsList[key].showCheckboxs = !this.groupJsList[key].showCheckboxs;
          } else {
            c.showCheckboxs = false;
          }
        })
        if (type == 'set') {
          this.groupKey = key;
          if (this.showCheckboxs) {
            let arr = this.groupJsList[key].jsList;
            setTimeout(() => {
              arr.forEach((b) => {
                this.data2.forEach((c) => {
                  c.children.forEach((v) => {
                    if (v.id == b.id) {
                      this.$refs.tree3.setChecked(v.key, true, false);
                    }
                  })
                })
              })
      ...
      getUpdateTearch(key) {
        ...
        const param = {code: key, rwid: this.rwid, zgh: zghArr};
        getUpdateTearch(param).then(response => {
      ...
      // 添加教师组
      addGroup() {
        this.groupJsList.push({showCheckboxs: false, jsList: []});
      },
      // 移除教师组
      delGroup(key) {
        let self = this;
        const param = {rwid: this.rwid, code: key};
        this
          .$confirm('是否删除此教师组?', '提示', {
          ...
          .then(() => {
            deleteGroupTeacher(param).then(response => {
              let {status, message} = response;
              if (status !== 200) {
              ...
              self.groupJsList.forEach((c) => {
                c.showCheckboxs = false;
              });
              self.contentList = [];
              self.groupJsList.splice(key, 1);
            })
          });
      },
      // 获取教师组列表
      getTearchGrouyList() {
        const param = {rwid: this.rwid};
        getTearchGrouyList(param).then(response => {
          ...
          if (data.length > 0) {
            ...
            for (let i = 0; i < data.length; i++) {
              let jsList = [];
              for (let j = 0, teacherList = data[i].teacherList; j < teacherList.length; j++) {
                jsList.push({
                  [teacherList[j].zgh]: [{
                    "jlid": '',
                    "rwid": this.rwid,
                    "zgh": teacherList[j].zgh,
                    "pkzs": '',
                    "pkjs": '',
                    "sfpk": '',
                    "xxdm": this.xxdm,
                  ...
                  name: teacherList[j].xm,
                  value: teacherList[j].zgh,
                  id: teacherList[j].zgh
                })
              }
              groupJsList.push({
                jsList,
            ...
            ;
            this.groupJsList = groupJsList;
          } else {
            this.groupJsList = [{showCheckboxs: false, jsList: []}]
          }
        });
      },
      // 查询指定课表数据接口
      getTearchGrouyBycodes(code) {
        this.groupKey = code;
        let data = {
          rwid: this.rwid,
          code
        ...
        this.contentList = [];
        let tempContentList = [];
        getTearchGrouyBycode(data).then(response => {
          let {data, status, message} = response;
          if (status !== 1) {
            this.$message.error(message);
            return 0;
          }
          let jcsjList = data.jcsjList;
          let swjs = data.attr.swjs;
          let xwjs = data.attr.xwjs;
          let wsjs = data.attr.wsjs;
          this.xqList = data.attr.pkzs.split(',');
          for (let i = 0; i < jcsjList.length; i++) {
            const course = {};
            if (jcsjList[i].sfgs === 1) {
              course.jcmc = jcsjList[i].jcmc;
              course.qzsj = jcsjList[i].qzsj;
              course.isMid = true;
              for (let j = 0; j < this.xqList.length; j++) {
                course.bpk = false;
                course.isRed = true;
            ...
            } else if (jcsjList[i].sfgs === 2) {
              course.jcmc = jcsjList[i].jcmc;
              course.jcdm = jcsjList[i].jcdm;
              course.qzsj = jcsjList[i].qzsj;
              for (let j = 0; j < this.xqList.length; j++) {
                const conlist = {};
                conlist.pkzs = this.xqList[j];
              ...
              let cell = jcsjList[i].cell;
              let pkzt = '';
              for (let n = 0; n < cell.length; n++) {
                if (cell[n]) {
                  course[cell[n].pkzs].jlid = cell[n].jlid;
                  course[cell[n].pkzs].sturts = cell[n].sturts;
                  if (cell[n].sfpk == 1) { // 1 固定排 2不排
                  
                    if (cell[n].sturts) {
                      course[cell[n].pkzs].ids = cell[n].ids;
                      pkzt = '固定排';
                      this.commonChange(
                        course[cell[n].pkzs],
                        pkzt,
      ...
      tree3Change(a,b) {
        let hash = {};
        this.checkSameTeacher(a,b)
        if (!a.node && b) {
          this.groupJsList[this.groupKey].jsList.push({
            [a.value]: [{
              "jlid": "",
              "rwid": this.rwid,
              "zgh": a.value,
          ...
          this.groupJsList[this.groupKey].jsList.forEach((c,i) => {
            if (c.id == a.id) {
              this.groupJsList[this.groupKey].jsList.splice(i,1)
            }
          })
        }
        let newArr = this.groupJsList[this.groupKey].jsList.reduceRight((item, next) => {
          hash[next.id] ? '' : hash[next.id] = true && item.unshift(next);
          return item
        }, []);
        this.groupJsList[this.groupKey].jsList = newArr;
      },
      // 勾选一个许璐，所有的许璐都应该被选中
      checkSameTeacher(target,bool) {
        this.data2.forEach((c) => {
          c.children.forEach((v) => {
            if (v.id == target.id) {
              this.$refs.tree3.setChecked(v.key, bool, false);
            }
          })
        })
      },
      // 移除教师组选中的教师
      delJsFromGroup(key, targetKey) {
        let id = this.groupJsList[key].jsList[targetKey].id;
        this.data2.forEach((c) => {
          c.children.forEach((v) => {
            if (v.id == id) {
              this.$refs.tree3.setChecked(v.key, false, false);
            }
          })
        });
        this.groupJsList[key].jsList.splice(targetKey, 1);
      },
      // 节点单击事件
      tree3Click(a, b) {
        if (!b.parent.parent) {
      ...
      clickChange(a) {
        const self = this;
        if (this.choiceType == 'batch') {
          let jsList = this.groupJsList[this.groupKey].jsList;
          if (jsList.length == 0) {
            this.$message.error("请选择教师");
            return;
          }
          if (!a.sturts) { // false不一致，true一致
            this.$message.error("课程不一致，无法进行操作!");
            return;
          }
        }
        if (this.num == 0 || this.num !== '') {
          async function saveTeacher() {
            ...
            if (self.choiceType == 'batch') {
              axiosName = saveBatchTearchCourse;
              let groupJsList = self.groupJsList[self.groupKey].jsList;
              let zghArr = [];
              groupJsList.forEach((c) => {
                zghArr.push(c.value)
              });
              param['code'] = self.groupKey;
              param['jlid'] = a.jlid;
              param['ids'] = a.ids;
              param['zgh'] = zghArr;
              // 1添加,2,修改,3删除
              //param['type'] = a.xspk == "" ? 1 : a.xspk == "不排课" ? 2 : 3;
              param['type'] = a.xspk == "" ? 1 : 3;
            } else {
              axiosName = saveTeacherpk;
              param['zgh'] = self.zghNode
              param['kcdm'] = self.kcdm
            }
            return new Promise(resolve => {
              resolve(
                axiosName(param).then(response => {
                  if (response.status !== 200) {
                    isErr = true;
                    self.$message.warning(response.message);
                  } else {
                    isErr = false;
                    if (self.choiceType == 'batch' && a.jlid == '') {
                      self.getTearchGrouyBycodes(self.groupKey)
                    }
                  }
                  return isErr;
                })
          ...
          async function deleTeacher() {
            ...
            if (self.choiceType == 'batch') {
              ...
              param['ids'] = a.ids;
              param['zgh'] = zghArr;
              param['type'] = 3; // 1添加,2,修改,3删除
            } else {
              axiosName = deleteTeacherpk;
              param['zgh'] = self.zghNode
              param['kcdm'] = self.kcdm
            }
...
<style scoped>
  ...
  .pkleftmenu .tab-box .tab-item {
    flex: 1;
    text-align: center;
    line-height: 40px;
  }

  .pkleftmenu .tab-box .tab-item.active {
    color: #fff;
    background-color: #409EFF;
  }

  .pkrightmenu .js-group-box li {
    margin-bottom: 10px;
  }

  .pkrightmenu .js-group-box .group-key {
    color: #606266;
    font-size: 14px;
    margin-right: 10px;
  }

  .pkrightmenu .js-group-box .js-list {
    min-height: 40px;
    padding-left: 20px;
    border: 1px solid #EBEEF5;
  }

  .pkrightmenu .js-group-box .js-item {
    position: relative;
    color: #606266;
    font-size: 14px;
    margin-right: 23px;
  ...
  .pkrightmenu .js-group-box .js-item .remove-js {
    position: absolute;
    right: -14px;
    top: 4px;
    cursor: pointer;
  ...
  .pkrightmenu .js-group-box .js-btn-box {
    width: 270px;
    margin-left: 10px;
  }

  .pkrightmenu .js-group-box .add-group-btn {
    color: #449AFC;
    font-size: 14px;
    cursor: pointer;
  }

  .pkrightmenu .js-group-box .add-group-btn span {
    font-size: 18px;
  }

  .pkrightmenu .js-group-box .del-group-btn {
    display: inline-block;
    color: #F66B6B;
    font-size: 14px;
    cursor: pointer;
  ...
  .pkrightmenu .js-group-box .del-group-btn span {
    display: inline-block;
