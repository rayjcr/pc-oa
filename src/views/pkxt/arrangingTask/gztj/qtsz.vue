  ...
  <div>
    <el-tabs v-model="activeName" :tab-position="tabPosition" @tab-click="indexKcsdList">
      <el-tab-pane label="课程时段分配" name="0">
        ...
        <el-alert v-if="selectedOptions[0]=='0'"
          style="margin: 4px 0 !important;"
          title="温馨提示，“灰色选中”原因：各年级课程时段设置不一致。"
          type="warning" show-icon>
        </el-alert>
        <el-table :data="tableData[0]" height="600"
    ...
    methods: {
      loadMain() {
        ...
        for (const o in this.options) {
          const i = parseInt(o);
          this.options[i] = [];
          this.options[i] = this.njList.map(e => {
        ...
        this.options[0].splice(0, 0, { value: '0', label: '全校' })
        this.selectedOptions.push('0');
        this.selectedOptions.push(this.options[1][0].value);
        this.selectedOptions.push(this.options[2][0].value);
        this.selectedOptions.push(this.options[3][0].value);
        /** 获取指定年级班级课程教师信息**/
      ...
      indexKcsdList() {
        ...
        /** 获取班级教学计划信息**/
          const pram = { rwid: this.rwid, njdm: this.selectedOptions[i] }
          getQtszList(pram, i).then(res => {
            /** 获取指定班级教学计划信息**/
            let nj_list;
            if (i === 0) {
              if (this.selectedOptions[i] === '0') {
                nj_list = _.filter(this.kcszList, item => item.zgh);
              } else {
                nj_list = _.filter(this.kcszList, item => item.njdm === this.selectedOptions[i] && item.zgh);
              }
              const [key, value] = [[], []]
              for (const o of nj_list) {
                if (!key.includes(o.kcdm)) {
                  key.push(o.kcdm);
                  value.push(o);
                }
              }
      ...
      formTodata(jsonData, query, i) {
        ...
        for (const o of query) {
          ...
          const map = {
            ...
            xxdm: this.xxdm,
            kcdm: o.kcdm,
            check: i === 0 ? this.checkTy(jsonData, { kcdm: o.kcdm }) : true,
