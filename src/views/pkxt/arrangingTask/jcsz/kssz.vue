        ...
        <!-- <el-card class="box-card-kssz">
          ...
          <div>
            <el-scrollbar style="height: 210px" class="scro">
              <el-tree class='pkbjtree' ref="tree" :data="treeData" node-key="id"  :props="defaultProps" :highlight-current="true" @node-click="handleTreeClick">
              </el-tree>
            </el-scrollbar>
          </div>
        </el-card> -->
        <el-card class="box-card-kssz">
          ...
          <div>
            <el-checkbox-group @change="handleWeek" v-model="weekGroup" size="small">
              <el-checkbox v-for="(item, index) in weeks" :label="index+1" :key="index" :disabled="disabledFlag">
                {{item}}
              </el-checkbox>
            </el-checkbox-group>
          </div>
        ...
        <el-card class="box-card-kssz">
          ...
          <div>
            <div class="pkset fl">
              ...
              <el-input-number v-model="num1" :min="0" :max="10" label="上午节数" size="small" :disabled="disabledFlag"></el-input-number>
            </div>
            <div class="pkset fl">
              <span class='kcspan'>下午</span>
              <el-input-number v-model="num2" :min="0" :max="10" label="下午节数" size="small" :disabled="disabledFlag"></el-input-number>
            </div>
            <div class="pkset fl">
              <span class='kcspan'>晚上</span>
              <el-input-number v-model="num3" :min="0" :max="10" label="晚上节数" size="small" :disabled="disabledFlag"></el-input-number>
            </div>
          </div>
        </el-card>
      </el-aside>
        ...
        <el-main class="el-main">
          <div class="view">
            <div class="rightitem">
              <div class="setKCtime">
                <div class="pkset fr">
                  <el-button icon="el-icon-document"  size="small" type="success" :disabled="disabledFlag" @click="createTableCheck">生成课表</el-button>
            ...
            <el-table id="KbMain-zhangzheng" :data="tableDataClass" :cell-class-name="handleTableItemClass" :span-method="handleClassTable" border style="width: 100%;" :row-class-name="tableRowClassName">
              ...
              <el-table-column v-for="(item, index) in copyWeekGroup" :key="index" align="center" :label="'星期' + formatWeek(item)">
                <template slot-scope="scope">
                  <!--@click="handleChange(scope.row['amount' + item])"-->
                  <div class="content" :class="{isforbidden: scope.row['amount' + item].gray}" >{{scope.row['amount' + item].name}}</div>
                </template>
              </el-table-column>
            </el-table>
    ...
    <el-dialog title="添加公共时间段" :visible.sync="selpublictime" width="500px">
      <el-form :model="publictimeCheck" ref="publictimeCheck2" label-width="90px">
        ...
        <el-form-item prop="selTime" resetField label="添加到" :rules="[{required: true,message: '时间段不能为空'}]">
          <el-select v-model="publictimeCheck.selTime" placeholder="请选择" style="width:250px;">
...
export default {
  ...
  data() {
    return {
      ...
      hello: false,
      disabledFlag:false,//是否置灰上课天数和上课节数
  ...
  methods: {
    ...
    async initKssz() {
      ...
      /** 获取班级**/
      api.whetherSetUpRules({
        rwid: this.rwid,
        xxdm: this.xxdm
      }).then(res=>{
        this.disabledFlag = res.data
      })
    ...
    async handleTableState(njdm, bjdm, pkzs, pkjs, state) {
      ...
      api
        ...
        .then(res => {
          ...
          //console.log(this.tableDataClass,77878777777777777777)
        })
        .then(() => {
