...
const BJ_KEY = 'pk_bj_list';
const NJ_KEY = 'pk_nj_list';
const KC_KEY = 'pk_kc_list';
const JS_KEY = 'pk_js_list';
const JSZ_KEY = 'pk_jsz_list';


// -------------------排课基础数据-------------------- 内部调用
function gradeList(query) {
  return fetch({
    url: '/api/course/pkGrade/getPkGradeList',
    method: 'get',
    params: query
  });
}
...
function classList(query) {
  return fetch({
    url: '/api/course/pkClass/getPkClassList',
    method: 'get',
    params: query
  });
}
...
function getKcAll(query) {
  return fetch({
    url: '/api/course/pkCourse/getPkCourseList',
    method: 'get',
    params: query
  });
}
...
function getJsAll(query) {
  return fetch({
    url: '/api/course/pkTeacher/getPkTeacherList',
    method: 'get',
    params: query
  });
}
...
export default {
  ...
  // 导出表格
  exportTable2Excel(oHtml, title, size) {
    let fontSize = 20;
    if (fontSize) {
      fontSize = size;
    }
