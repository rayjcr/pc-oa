        ...
        <table cellpadding="0" cellspacing="0" class="gentable fl mt10 tzkcTable">
          ...
          <template v-for="(item, rowIndex) in tableCellDataSet">
            ...
            <tr :key="'row-'+rowIndex">
              ...
              <td v-for="(element,colIndex) in timeTable.daysPerWeek" :key="'col-'+colIndex"
                  @click.stop="bj_cellClick(tableCellDataSet[rowIndex][colIndex])"
          ...
          <tr>
            ...
            <td v-for="(element,colIndex) in timeTable.daysPerWeek" :key="'zcq-0-'+colIndex"
                @click.stop="bj_cellClick(zcqDataSet[0][colIndex])"
          ...
          <tr>
            <td v-for="(element,colIndex) in timeTable.daysPerWeek" :key="'zcq-1-'+colIndex"
                @click.stop="bj_cellClick(zcqDataSet[1][colIndex])"
...
//      this.selectNj = this.gradeDataList[0].njdm;
      ...
      bj_cellClick(item) {
        ...
        if (this.curActive !== null) {
          if (this.curActive.index === item.index && this.curActive.ctype === item.ctype) {
            ...
            if (item.state === 1||item.state === 2) { // 可安排
              const from = this.curActive;
              const to = item;
              this.curActive = null;
              const records = pktz.swapCourse(from, to,this.coursePlanDataSet,this.timeTable);
              pktz.showMoveDetail(this,records);
      ...
      bj_computEnableCell() {
        ...
        const enableCellDataSet = bjtz.computEnableCell(mainTableDataSet, initContent, this.ruleDataSet, this.curActive, this.timeTable,
          this.coursePlanDataSet,this.ignore_role);
        this.tableCellDataSet = bjtz.getSwapCourseList(enableCellDataSet, mainTableDataSet);
        // 计算暂存区可对调课程
        const zcqMainTable = this.getZcqContentByClassId(this.selectBj);// 获取班级暂存区数据
        const initZcqContent = pktz.initZcqStructure(this.timeTable);//初始化暂存处表格
