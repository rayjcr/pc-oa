      ...
      <!-- <el-tab-pane :label="item.bjmc" v-for="(item,index) in dataList" :key="index"> -->
        ...
        <el-table
          ...
          <el-table-column :label="item2.mc" v-for="(item2,index2) in tableHeader" :key="index2" align="center">
            ...
            <el-table-column label="项目等级" align="center" v-if="item2.zhdj !== 0">
              <template slot-scope="scope">
                <span>{{[scope.row.xmReportList,item2.uuid, 'dj'] | scorefilter3}}</span>
              </template>
            </el-table-column>
            <el-table-column label="项目积分" align="center" v-if="item2.zhxf!==0">
              <template slot-scope="scope">
      ...
      for(let i = 0; i < rwList.length; i++) {
        if (!rwList[i].ywzx) {
          if (rwList[i].jflx === 0) { // 成绩换算
            ...
            for(let j = 0; j < rwList[i].rwList.length; j++) {
              if(rwList[i].rwList[j].uuid == xmid && rwList[i].rwList[j].xmcjInfo.twInfo) {
