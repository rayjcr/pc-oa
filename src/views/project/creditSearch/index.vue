    ...
    <div class="app-container calendar-list-container">
        ...
        <div class="u_TableMain">
            <div class="filter-container">
                <div class="select_box"><span class="filterTit">学年学期</span> 
                    <el-select  class="schoolYear" v-model="listQuery.xn" placeholder="请选择" @change="getList(listQuery.xn,'xn')">
                        <el-option
                    ...
                    <el-select  class="schoolYear" v-model="listQuery.xq" placeholder="请选择" @change="getList(listQuery.xq,'xq')">
                        <el-option
                ...
                <div class="select_box"><span class="filterTit">年级班级</span>
                    <el-select  class="schoolYear" v-model="listQuery.njdm" placeholder="请选择" @change="getList(listQuery.njdm, 'njdm')">
                        <el-option
                    ...
                    <el-select  class="schoolYear" v-model="listQuery.bjId" placeholder="请选择"  @change="getList(listQuery.bjId, 'bjdm')">
                        <el-option
                ...
                <div class="select_box"><span class="filterTit">科目</span>
                    <el-select  class="schoolYear w200" v-model="curriculumIndexList" filterable multiple collapse-tags clearable placeholder="请选择" @change="getList(curriculumIndex, 'kcdm')">
                        <el-option
                    ...
                    <el-input style="width: 150px;" v-model="searchInfo" placeholder="请输入姓名" @blur="getList(searchInfo, 'ssxx')"> </el-input>
                    <el-button style="margin-left: 10px;"  type="primary" icon="edit" @click="exporttable" :loading="downloads">导出</el-button>
                </div>
            </div>
            <!-- 顶部end -->
            <!-- 表格区域beg -->
            <div class="table-container" style="min-width:900px;">
    ...
    methods: {
        ...
        init() { // 初始化
            ...
            Promise.all([
                ...
                _this.curriculumIndexList = [0];
                // _this.getAllBj()
                // _this.classlist = _this.getAllBj()
                setTimeout(() => {
                    _this.getList('', 'start')
                }, 100)
            });
        },
        async getList(datas,str) {
            if (datas === '') {
            ...
            var kcmcList = [];
            var kcidList = [];
            this.curriculumIndexList.forEach((e)=>{
              kcidList.push(this.curriculumlist[e].uuid)
              kcmcList.push(this.curriculumlist[e].kcmc)
            })
            ...
            // this.listQuery.kcId = this.curriculumIndex !=="" ? this.curriculumlist[this.curriculumIndex].uuid : "";
            // this.listQuery.kcmc = this.curriculumIndex !=="" ? this.curriculumlist[this.curriculumIndex].kcmc : "";
            this.listQuery.kcId = kcidList.length>0 ? kcidList.join(',') : "";
            this.listQuery.kcmc = kcmcList.length>0 ? kcmcList.join(',') : "";
            if (this.classlist.length) {
                this.listQuery.bjId = (datas === '')?this.classlist[0].uuid:((str === 'bjdm')?datas:this.classlist[0].uuid)
                let bjidArr = [];  //班级代码value数组
                for(let i = 0; i < this.classlist.length; i++) {
                    bjidArr.push(this.classlist[i].uuid) 
                }
                this.classidArr = JSON.parse(JSON.stringify(bjidArr))
