...
<div class="app-container calendar-list-container body">
        ...
        <div class="m_GenTableBox">
            ...
            <el-row :gutter="20"  class="box-table">
                ...
                <el-col :span="6" class="rw-table">
                    ...
                    <div class="rw-body">
                        <el-table :data="rwList" @row-click="selectBj" ref='rwTable' highlight-current-row :row-class-name="tableRowClassName" v-loading.body="listLoading">
                            <el-table-column width="3"></el-table-column>
                            <el-table-column width="20"></el-table-column>
                            <el-table-column label="名称">
                                <template slot-scope="scope">
                                    <i class="el-icon-star-on rwstar" v-if="scope.row.xjrw==1"></i>
                                    <span v-if="itemDetail.xmlx == 0 || itemDetail.xmlx == 1 || itemDetail.xmlx == 4">{{scope.row.mc}}</span>
                ...
                <el-col :span="18" class="cj-table">
                    <div class="cj-header" v-if="((bjcj.length>0 && itemDetail.xmlx==0)||(resultData.length>0 && itemDetail.xmlx!=0))">
                        ...
                        <div v-if="itemDetail.jflx != 6">
                            ...
                            <el-button type="primary" class="right mright" @click="importData">数据导入</el-button>
                        ...
                        <el-tabs type="border-card" @tab-click="tabClick" v-model="cjIndex" v-if="(bjcj.length>0 && itemDetail.xmlx==0)||(resultData.length>0 && itemDetail.xmlx!=0)">
                            ...
                            <el-tab-pane :label="item.bjmc" v-for="(item,index) in bjcj" :key="index">
                                <div class="table-header">
                                    ...
                                    <div v-if="itemDetail.jflx == 3">录入说明：1=A等级；2=B等级；3=C等级；4=D等级；5=待评；</div>
                                </div>
                                <div class="table-body">
                                    <el-table class="levelTable" :data="item.data" border style="width: 100%"            v-if="itemDetail.jflx != 4">
                                        ...
                                        <el-table-column label="加减值" v-if="itemDetail.jflx==2">
                                            <template slot-scope="scope">
                                                <el-input-number @change="changeXf(scope.row)" v-inputDisabled v-model="scope.row.xf" :precision="1" :step="currentRw.zl" :max="currentRw.xf"></el-input-number>
                                            </template>
                                        </el-table-column>
                                        ...
                                        <el-table-column label="数值" v-if="itemDetail.jflx==5">
                                            <template slot-scope="scope">
                                                <div class="inpBSign">
                                                    ...
                                                    <el-input class="smallipt" v-model="scope.row.sz" placeholder="请输入内容" clearable @blur="inpBlur($event,scope.row)" @clear="inpClearsz($event,scope.row)"></el-input>
                                                    

                                                    <el-select @change="inpBlur($event,scope.row)" class="smallipt" v-model="scope.row.kszt" placeholder="请选择">
                                                        <el-option
                                    ...
                                    <el-tabs v-if="itemDetail.jflx == 4"  v-model="activeStuName">
                                        <el-tab-pane :label="student.xm" v-for="(student, index2) in item.data" :key="index2" :name="student.index">
                                            ...
                                            <el-row class="twIndRow">
                                                <el-button type="primary" @click="twSure(item.data)" >保 存</el-button>
                                            </el-row>
                                        </el-tab-pane>
                                    </el-tabs>
                                    <div v-if="itemDetail.jflx == 6">
    ...
    <el-dialog :title="textMap[dialogStatus]"  :visible.sync="createFormVisible" width="600px" :before-close="createCancel">
        ...
        <el-form :model="form" ref="createform" label-width="120px" :rules="rules">
            ...
            <el-form-item label="任务最高值" prop="xf" v-if="itemDetail.jflx==0 || itemDetail.jflx==7||itemDetail.jflx==2 ">
                <el-input v-model="form.xf" :disabled="isChange"></el-input>
            </el-form-item>

            <!-- 添加排序end -->
            <el-form-item label="默认值" required v-if="itemDetail.jflx == 2">
                <el-radio v-model="defaultXf" label='0'>最低分(0分)</el-radio>
                <el-radio v-model="defaultXf" label='1'>最高分(任务最高值)</el-radio>
            </el-form-item>
            <el-form-item label="加减幅度" v-if="itemDetail.jflx == 7||itemDetail.jflx == 2" prop="zl">
            ...
            <el-form-item label="录入人员" required>
                <el-checkbox-group v-model="lrrylxList">
                    ...
                    <el-checkbox label='4'>家长</el-checkbox>
                </el-checkbox-group>
            </el-form-item>
            ...
            <el-form-item label="星级任务" required v-if="isStarRw">
                <el-radio v-model="xjrw" label='1'>是</el-radio>
                <el-radio v-model="xjrw" label='0'>否</el-radio>
            </el-form-item>
    ...
    <el-dialog title="任务导入" :visible.sync="BatchimportCjFormVisible" width="540px" :before-close="BatchimportCancel" class="import_box">
...
import {
    ...
    getBj,
    GetXmStatistics,
    CheckXtResult, BatchInputValue ,UploadPictures,DeleteCreditValue,CurrencyReport,rwBatchexportExcel,
    // importExcel,
    getGrantedClass
...
export default {
    ...
    data() {
        return {
            ...
            lguserType: window.localStorage.getItem('lguserType'),             //用户类型     3:普通老师
            defaultXf:'0',                            //默认最小值， 0最小值  1最大值
            errData:[],                             //错误数据列表
            stateList: [{"key":"正常","value":0},{"key":"补考","value":1},{"key":"缺考","value":2},{"key":"病假","value":3},{"key":"请假","value":4}],
            isStarRw:false,                         //是否可以建立星级任务
            xjrw:"0",                               //是否是星级记录
            curItemLabel:"",                        //当前选中的班级名称
        }
    },
    created() {
        this.dqXnXqObj();
        ...
        zlRange: function() {
            ...
            if(this.form.xf<=200){
                for(let i = 1; i <= this.form.xf; i++) {
                    if(this.form.xf % i == 0) {
                        zlArr.push(i.toString())
                    }
                }
            }else{
                this.tipsXX("任务最高值为200",'warning');
                // this.form.xf = 200;
            }
            /***
             * 2019-06-25屏蔽，暂不清楚为什么判断当前任务的学分是否等于表单的学分 不等于后zfzl为空
             */
            // if(this.currentRw && this.currentRw.xf){
            //     // console.log("this.currentRw.xf="+this.currentRw.xf);
            //     // console.log("this.form.xf="+this.form.xf);
    ...
    methods: {
        ...
        getItemDetail() {
            getItem({
                ...
                console.log(this.itemDetail,650);
                // 判断是否为星级任务
                if(response.realZhdj==5){
                    this.isStarRw = true;
                }
        ...
        //加载所在年级的所有班级     加载所在老师下的
        getAllBj () {
            if(this.lguserType != 3){
                getBj({
                    page: 1,limit: 500,
                    xxdm: this.listQuery.xxdm,
                    njdm: this.itemDetail.njdm,
                    jbny: this.itemDetail.xn,
                }).then(response => {
                ...
                getGrantedClass({
                    ...
                    njdm: this.itemDetail.njdm,
                    xndm: this.xnxq.xn,
                    zgh : window.localStorage.getItem('usercode')
                }).then(response => {
        ...
        getList() {
            page(this.listQuery).then(response => {
                ...
                if(this.rwList && this.rwList[0] && this.rwList[0].uuid) {
                    ...
                    this.$refs.rwTable.setCurrentRow(this.rwList[0]);
                }                
        ...
        getBjCj(rwid) {
            ...
            getCreditList({
                ...
                for(let i = 0; i < this.bjcj.length; i++) {
                    ...
                    if(that.curItemLabel){
                        var indexnum = _.findIndex(this.bjcj,{'bjmc':that.curItemLabel});
                        // console.log("indexnum="+indexnum);
                        if(indexnum==-1){
                            that.cjIndex = '0';
                            that.curItemLabel = '';
                        }else{
                            that.cjIndex = _.findIndex(this.bjcj,{'bjmc':that.curItemLabel}) + '';
                        }
                    }else{
                        that.cjIndex = '0';
                    }
        ...
        createSubmit(formName) {
            ...
            this.$refs[formName].validate(valid => {
                if (valid) {
                    ...
                    if(_this.itemDetail.jflx==2){
                        this.form.mrxf = this.defaultXf=='0'?0:this.form.xf;
                        this.form.zl = this.zfzl;
                    }
                    ...
                    //是否是星级任务
                    this.form.xjrw = parseInt(this.xjrw);
        ...
        handleUpdate(item) {
            ...
            getRw({id:item.uuid}).then(res => {
                ...
                that.form.bjid = res.bjid;
                that.form.mc = res.mc;
                that.form.px = res.px;
                that.isChange = res.rwyssjCount>0?true:false;       //任务是否能被修改
                that.$set(that.form,'xf',res.xf);
                that.form.zl = that.zfzl = that.zfzl = res.zl;
                console.log(res,1000);
                console.log(that.zfzl,1001);
                that.sfggrw = String(res.sfggrw)               //是否公共任务
                that.bjidSelect = that.form.bjid.split(',');

                that.bjidBeifen = that.bjidSelect;
                let bjid = [];
                for(let i = 0; i < that.bjidSelect.length; i++) {
                    let index = that.bjidArr.indexOf(that.bjidSelect[i])
                    if(index>=0) {
                        bjid.push(that.bjList[index])
                    }
                }
                that.bjid = bjid;
                // 录入人员
                ...
                if(res.lrrylx==3){
                    ...
                    that.lrrylxList = ['1','2','4']
                }
                that.createFormVisible = true;
            })
        },
        //删除任务
        handleDelete(item) {
        ...
        tabClickName(item){
            ...
            console.log(this.bjcj[0].data[item.index]);
        },
        //任务列表项选中事件
        ...
        // 录入--积分
        inpBlur(ev,row){
            // console.log(row,1017)
            // console.log(this.itemDetail.jflx,1018)
            if(!(row.sz==="") && this.itemDetail.jflx!=5){
                ...
                var batchList = []
                batchList.push(row)
                console.log(batchList,111)
                this.tBatchInputValue(batchList)
            }else if(!(row.sz==="") && this.itemDetail.jflx==5){
                console.log(222)
                var batchList = []
                batchList.push(row)
                this.tBatchInputValue(batchList)
        ...
        // 录入--等级
        down2next(event,raw){
            console.log(event,1114);
            var e = raw.next
            var curData = raw.curData
            var all = raw.numAll
            var code = event.keyCode || event.which || event.charCode;
            var rawCopy = JSON.parse(JSON.stringify(raw))
        ...
        //图文保存
        twSure(student){
            //console.log(student,1335);
            // console.log(student,1333)
            // var isGserr = false
        ...
        querySearch(queryString, cb) {
            var stateTemp = this.stateList;
            var results = queryString ? this.createFilter(queryString) : stateTemp;
            // // 调用 callback 返回建议列表的数据
            console.log(results,1520)
            console.log(queryString,1521)
            // cb(results);
            cb(results);
        ...
        createFilter(queryString) {
            ...
            this.stateList.forEach(function(item,index){
                console.log(item,1529)
                console.log(queryString,1530);
