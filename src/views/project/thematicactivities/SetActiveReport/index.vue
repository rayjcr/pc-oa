 ...
 * @Date: 2019-03-15 10:19:30
 * @LastEditTime: 2019-04-24 09:59:45
 -->

<template>
    <div class="app-container" @click.stop="closeqxgl">
                ...
                <el-form :model="formSearch" ref="Searchform" label-position="left" class="left">
                    <el-row style="min-width:600px; padding-left:20px;">
                        <el-form-item label="学年学期">
                            <el-select v-model="formSearch.xn" clearable placeholder="选择学年" @change="searchFn('search')">
                                <el-option v-for="(item,index) in xnList" :key="index" :label="item.xn" :value="item.xn"></el-option>
                            </el-select>
                            <el-select v-model="formSearch.xq" clearable placeholder="选择学期" @change="searchFn('search')">
            ...
            <div class="m_GenTableBox">
                <div class="t_conmain"  v-loading.body="listLoading">
                    ...
                    <el-card class="partbox-card" shadow="hover" v-for="(item,index) in reportList" :key="index">
                        ...
                        <div class="rightpart">
                            <div class="sole">
                            <el-button type="text" @click.stop="Jurisdiction(index,item)">报告查看权限</el-button>
                            <div class="check_box" v-show="showindex === index">
                                <el-checkbox v-model="ckqx.xsqx" @change="allowstudent($event,item)">允许学生查看报告</el-checkbox>
                            </div>
                            </div>
                            <div class='opt'>
        ...
        <!-- 新建 -->
        <el-dialog :close-on-click-modal="false" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="650px" :before-close="dialogCancel" class="dialogprog">
...
export default {
    ...
    data() {
      return {
        ckqx:{
            xsqx: false, // 是否授予学生查看权限
        },
        // checked: false, // 是否授予学生查看权限
        showindex: null, // 是否显示权限选择框
        formSearch:{},          //搜索
        xnList:[],              //学年
        xqList:[],              //学期
        listQuery:{                
    ...
    methods: {
      ...
      /**
       * @description: 权限选择器显隐控制
       * @param {type} 
       * @return: 
       */  
      Jurisdiction(index,item){
          this.ckqx.xsqx = item.ckqx===2?true:false
      ...
      DictObj() {
          ...
          reportapi.GetReportTemplateList({xxdm: this.listQuery.xxdm, zyy: 2,zsggmb: true}).then(response => {
            //   console.log(response)
      ...
      handleCreate(){
          ...
          this.form.mbid = this.mbList[0].uuid
          this.form.lx = 2
      },
      //取消弹层
      dialogCancel(){
      ...
      handleDelect(item){
        ...
        // if (!item.byzD1.length) {
            this.$confirm('确认删除此报告单吗？删除后无法恢复，请谨慎操作！', '提示', {
                ...
                // console.log('5555')
                avtive.delBgds({id: item.uuid}).then(data => {
                    // console.log(data)
                    // if (data === '删除成功。') {
        ...
        // }  else {
        //     this.tipsXX("已录入数据，不可删除！",'warning','失败');
        // }         
      },
      //复制
      handleCopy(item){
      ...
      handleSet(item){
          ...
          this.$router.push({
              path: '/thematicactivities/reportSet', 
          })
      },        
      //提示信息
...
<style scoped lang="scss">
  ...
  .partbox-card{
      ...
      .rightpart{
          ...
          /* position: relative; */
          .sole{
              height: 40px; line-height: 40px; color: #409EFF;
          }
          .check_box{
            width: 150px;
            height: 55px;
            ...
            box-shadow:  5px 6px 16px 6px rgba(0,0,0,0.2);
          }
          .opt{
