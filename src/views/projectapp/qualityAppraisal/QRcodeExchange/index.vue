        ...
        <el-form ref="downloadform" :rules="downloadRules" :model="downloadInfo">
          ...
          <el-form-item label="尺寸：" prop="QRcodeSize">
            <el-input v-model="downloadInfo.QRcodeSize" maxlength="4" class="w200"
                      placeholder="请输入50-1000的数字"></el-input>
          </el-form-item>
        </el-form>
      </div>
  ...
  export default {
    ...
    data() {
      ...
      const one100to1000 = (rule, value, callback) => {
        ...
        if (!reg.test(value)) {
          callback('请输入50-1000之间的数字');
        } else {
          if (Number(value) > 1000 || Number(value) < 50) {
            callback('请输入50-1000之间的数字');
          } else {
            callback();
          }
      ...
      return {
        ...
        downloadRules: {
          ...
          QRcodeSize: [{required: true, message: '尺寸请输入50-1000的数字', tigger: 'blur'},
