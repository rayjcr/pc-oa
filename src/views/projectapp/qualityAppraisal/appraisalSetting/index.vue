<template>
    <div class="app-container calendar-list-container">
        <el-tabs class="u_TableMain columns ofhide" v-model="activeIndex" @tab-click="getEvaluation(1)" type="border-card">
                    ...
                    <el-table class="tableBorder dimensionTable" :data="tableData">
                        ...
                        <el-table-column label="操作" width="180" align="center">
                            <template slot-scope="scope">
                                ...
                                <el-popover placement="top" width="160" v-model="scope.row.deltips_2">
                                    <p>是否删除此评价项</p>
                                    <div style="text-align: right; margin: 0">
                                        <el-button size="mini" type="text" @click="scope.row.deltips_2 = false">取消</el-button>
                                        <el-button type="primary" size="mini" @click="deleteEvaluation(scope.row)">确定</el-button>
                                    </div>
                                    <el-button slot="reference" type="danger" icon="el-icon-delete" size="mini">删除</el-button>
                                </el-popover>
            ...
            <el-tab-pane name="3">
                <span slot="label"><i class="el-icon-trophy"></i> 悬赏任务</span>

                <div class="filter-container innerTab">

                    <el-button size="small" icon="el-icon-plus" type="primary" @click="editRewardTask('create')">新增悬赏任务</el-button>
                    <!-- <el-button size="small" type="primary" @click="bgdInput()">批量导入</el-button> -->
                </div>
                ...
                <div class="m_GenTableBox">
                    <el-table class="tableBorder dimensionTable" :data="tableData">
                        ...
                        <el-table-column label="活动时间" prop="startTime">
                            <template slot-scope='scope'>
                                {{scope.row.startTime}}-{{scope.row.endTime}}
                            </template>
                        </el-table-column>
                        <el-table-column label="子任务数" prop="subTaskCount">
                        </el-table-column>
                        <el-table-column label="学生最多领取" prop="maxCount">
                            <template slot-scope='scope'>
                                {{scope.row.maxCount}}个子任务
                            </template>
                        </el-table-column>
                        <el-table-column label="最新修改" prop="updateTime" width="140">
                        </el-table-column>
                        <el-table-column label="操作" align="center">
                            <template slot-scope="scope">
                                <el-button type="text" style="margin-left: 0;" size="small" @click="editRewardTask('update',scope.row)">编辑</el-button>
                                <el-button type="text" style="margin-left: 0;" size="small" @click="subTaskManage(scope.row)">子任务管理</el-button>
                                <el-button type="text" style="margin-left: 0;" size="small" @click="clickStatistics(scope.row,currentPage)">统计</el-button>
                                <el-popover placement="top" width="160" v-model="scope.row.deltips_3">
                                    <p>是否删除此评价项</p>
                                    <div style="text-align: right; margin: 0">
                                        <el-button size="mini" type="text" @click="scope.row.deltips_3 = false">取消</el-button>
                                        <el-button type="primary" size="mini" @click="deleteEvaluation(scope.row)">确定</el-button>
                                    </div>
                    ...
                    <el-table class="tableBorder dimensionTable" :data="tableData">
                        ...
                        <el-table-column label="操作" align="center">
                            <template slot-scope="scope">
                                ...
                                <el-popover placement="top" width="160" v-model="scope.row.deltips_4">
                                    <p>是否删除此惩罚项</p>
                                    <div style="text-align: right; margin: 0">
                                        <el-button size="mini" type="text" @click="scope.row.deltips_4 = false">取消</el-button>
                                        <el-button type="primary" size="mini" @click="deleteEvaluation(scope.row)">确定</el-button>
                                    </div>
                                    <el-button slot="reference" type="danger" icon="el-icon-delete" size="mini">删除</el-button>
                                </el-popover>
            ...
            <div class="pjProjectMain">
                <el-form :model="personEaluationForm" ref="form" :rules="personEaluationRule" label-width="100px">
                    <el-form-item label="类型：" prop="type">
                        <el-radio :disabled="dialogStatus=='update'" v-model="personEaluationForm.type" label='1'>录入型</el-radio>
                        <el-radio :disabled="dialogStatus=='update'" v-model="personEaluationForm.type" label='2'>任务型</el-radio>
                    </el-form-item>

                    <el-form-item v-if="personEaluationForm.type=='2'" label="任务类型：" prop="taskType">
                        <el-radio :disabled="dialogStatus=='update'" v-model="personEaluationForm.taskType" :label='1'>常规任务</el-radio>
                        <el-radio :disabled="dialogStatus=='update'" v-model="personEaluationForm.taskType" :label='2'>考勤打卡</el-radio>
                        <!-- <el-radio :disabled="dialogStatus=='update'" v-model="personEaluationForm.taskType" :label='3'>悬赏任务</el-radio> -->
                    </el-form-item>
                    ...
                    <el-form-item label="图标：" class="lh50" prop="icon"  v-if="personEaluationForm.taskType==1||personEaluationForm.type==1">
                    ...
                    <el-form-item label="维度：" prop="dimensionId" v-if="personEaluationForm.taskType!==3||personEaluationForm.type==1">
        ...
        <!--添加编辑悬赏任务弹层-->
        <el-dialog :title="textMap[dialogStatus]+'悬赏任务'" :close-on-click-modal="false" :visible.sync="dialogFormVisibleReward" width="600px">
            <div class="pjProjectMain">
                <el-form :model="personEaluationRewardForm" ref="rewardform" :rules="personEaluationRewardTaskRule" label-width="100px">
                    <el-form-item label="名称：" prop="name">
                        <el-input class="w300" placeholder="请输入1~24字" size="small" maxlength="48" v-model="personEaluationRewardForm.name"></el-input>
                    </el-form-item>

                    <el-form-item label="领取限制：" prop="maxCount" class="must must4">
                        学生最多领取
                        <el-input v-model="personEaluationRewardForm.maxCount" size="small" maxlength="1" class="w60" placeholder="1~9"></el-input>
                        个子任务
                    </el-form-item>
                </el-form>
            </div>
            <div slot="footer" class="dialog-footer">
                <el-button @click="cancel(personEaluationRewardForm,'rewardform')">取 消</el-button>
                <el-button type="primary" :loading="submitLoading" @click="submitRewardTask('rewardform')">确 定</el-button>
            </div>
        </el-dialog>
...
export default {
    ...
    data() {
        ...
        var benchmarkCheck = (rule, value, callback) => {
            var base,max,min;
            if(value.benchmark!='' && value.benchmark){
                var reg =  /^[1-9]\d*$|^0$/;
        ...
        var rewardTaskMaxCount = (rule, value, callback) => {
            var reg =  /^[1-9]$/;
            if(value == ''){
                callback('请输入领取限制');
        ...
        return {
            ...
            personEaluationRewardForm: {name:'2',maxCount:''},//个人评价项悬赏任务表格
            personEaluationRule:{
            ...
            personEaluationRewardTaskRule:{
                name: [{ required: true,message: '请输入评价项名称',tigger: 'blur'},{max:24,min:1,message:'请输入1-24个字符'}],
                maxCount: [{ required: true,validator: rewardTaskMaxCount,tigger: 'blur'}]
            },
    ...
    beforeRouteEnter (to, from, next){
        // 如果是统计页面返回就继续下步，否则清空session里的tabpan
        if(from.path.indexOf('statistics')!=-1){
            next();
        }else{
            window.sessionStorage.removeItem("tabpan")
            next();
        }
    },
    mounted(){
        // console.log(this.$router,"路由路径")

        if(window.sessionStorage.getItem('tabpan')){
            let tabObj = JSON.parse(window.sessionStorage.getItem('tabpan'));
            this.activeIndex = tabObj.tabpan.toString();
        }
        this.getDimension();
        this.getEvaluation();
        this.getConfig();
    ...
    //通用
        ...
        //分页获取评价项
        async getEvaluation(page,limit){
            var params = {
            ...
            this.tableData = []
            var queryApi = this.activeIndex!='3'?api.getEvaluationByPage:api.getEvaluationItemCategory;
            var activeIndex = _.clone(this.activeIndex)
            var res = await queryApi(params)
        ...
        cancel(val,formName){
            ...
            this.dialogFormVisibleReward = false;//关闭弹框
        ...
        deleteEvaluation(row){
            console.log(row)
            var queryApi = this.activeIndex=='3'?api.delEvaluationItemCategory:api.deleteEvaluation;
            queryApi(row.id).then(res => {
                if(res.resultCode==1){
                    this.dialogFormVisible = false;//关闭弹框
                    this.$notify({
                        type: 'success',
        ...
        changeEvaluationList(type,val){
            ...
            if(type=='create'){
                ...
                api.getEvaluation(val.id).then(res => {
                    ...
                    if(this.personEaluationForm.taskType!=2){
                        this.$set(this.personEaluationForm,'evaluationItemRuleAttendance',{type:''});
                    }
    ...
    // 悬赏任务
        // 增加或修改悬赏任务
        editRewardTask(type,val){
            this.dialogStatus = type;
            if(this.$refs.rewardform){
            this.personEaluationRewardForm = {
                name: '',
                maxCount: '',
            ...
            this.dialogFormVisibleReward = true;
        },
        // 提交悬赏任务
        submitRewardTask(){
            var apiQuery = this.dialogStatus=='create'?api.addEvaluationItemCategory:api.editEvaluationItemCategory;
            this.$refs.rewardform.validate((valid) => {
            if(valid){
                this.submitLoading = true;
                apiQuery(this.personEaluationRewardForm).then(res=>{
                if(res.resultCode==1){
                    this.dialogFormVisibleReward = false;
                    this.$notify({
                        type: 'success',
                        message: '操作成功！',
                        time: 1000
        ...
        subTaskManage(row){
            this.$router.push({
                path:'/qualityAppraisal/appraisalSetting/subtaskManagement',
                query:{
                    categoryId: row.id
                }
            });
        ...
        //统计 
        clickStatistics(row,currentPage){
            window.localStorage.setItem("qualityname",row.name)
            this.$router.push({
                path:'/qualityAppraisal/appraisalSetting/statistics',
                query:{
                    categoryId: row.id,
