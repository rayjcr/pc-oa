...
export default {
    ...
    methods: {
        async init(){
            ...
            if(this.courseID&&(this.allCourse!='all')){
                this.excludeEval();
            }
            this.compose();
        },
        ...
        excludeEval(){
            for(var i=0;i<this.allTeacherCheck.length;i++){
                this.allEvaluateList = _.reject(this.allEvaluateList,{"id":this.allTeacherCheck[i].id});
            }
            // console.log(this.allEvaluateList,'123123')
        },
        filterEvaluateList(){
