<template>
    <div class="app-container">
        ...
        <el-dialog :title="textMap[dialogStatus]+'角色'" :close-on-click-modal="false" :visible.sync="roleDialog" width="550px">
            <el-form :model="roleForm" :rules="rules" ref="form" label-width="100px">
                ...
                <el-form-item label="关联班级：">
                    <el-cascader class="w300 selClass" size="small" placeholder="选择班级" v-model="roleForm.classCode" :options="GradeClassList" filterable :props="{ children:'bjList', expandTrigger: 'hover', multiple: true}" collapse-tags clearable></el-cascader>
                </el-form-item>
        ...
        <el-dialog title="配置权限" :close-on-click-modal="false" :visible.sync='roleSetDialog' width="800px">
            <!--自定义角色配置-->
            ...
            <div v-if="curEditRole.type==4&&isReShow">
                <el-tabs  v-model="activeTeacherHonor" tab-position="left" class="tabsMain">
                    <el-tab-pane label="基础权限">
                        <div class="tabCont">
                            <div class="setTit">基础权限</div>
                            <div class="setCont" v-if="permissionList.length">
                                <el-checkbox v-model="permissionList[0].permissions[1].isPermitted">班级动态显示其他教师评价</el-checkbox>
                            </div>
                            <el-button @click="submitPermission" type="primary" size="small" class="tabSubmit">提交</el-button>
                        </div>
                    </el-tab-pane>
...
export default {
    ...
    data() {
        return {
            ...
            roleForm:{roleName:'',summary:'',classCode:''},                    //角色新建、修改表单对象
            rules: {
                roleName: [
    ...
    methods: {
        ...
        async getAllGradeClass(){
            ...
            console.log(this.GradeClassList,"this.GradeClassList")
        ...
        editTeacher(item){
            ...
            this.roleForm.classCode = [];
            for(let i=0;i<item.classList.length;i++){
                this.roleForm.classCode.push([item.classList[i].gradeId,item.classList[i].classId])
            }
            // this.roleForm.classCode = item.classList;
        ...
        async submitSaveRole(){
            ...
            for(var i=0;i<this.checkTeacher.length;i++){
                param.members.push({'teacherId':this.checkTeacher[i].gh,'name':this.checkTeacher[i].xm,'xxdm':localStorage.getItem("xxdm")});
            }
            console.log(this.roleForm.classCode,"this.roleForm.classCode")
            if(this.roleForm.classCode){
                for(let j=0;j<this.roleForm.classCode.length;j++){
                    param.classList.push({"classId":this.roleForm.classCode[j][1],"gradeId":this.roleForm.classCode[j][0],"createYear":this.xnxqObj.xn});
                }
            }
            // console.log(this.roleForm.classCode,"this.roleForm.classCode")
            // console.log(param,'paramSave')
