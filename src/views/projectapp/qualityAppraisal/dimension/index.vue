        ...
        <div class="u_TableMain">
            ...
            <div class="m_GenTableBox">
              ...
              <!--                  <i class="delfont el-icon-check" style="color:#67C23A" @click="saveDimension(scope.row)"></i>
                            ...
                            <span v-else>
                                 {{scope.row.name}} 
                                 <!-- <i class="dimensionEdit el-icon-edit" @click="editDimension(scope.row)"></i> -->
                            </span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="dimensionType" label="维度类型" align="center">
                        <template slot-scope="scope">
                           
                            <div class="inline" v-if="scope.row.level==1&&(!scope.row.dimensionType||scope.row.edit)">
                                <el-select v-model="scope.row.dimensionType" style="width:120px;" placeholder="请选择" size="mini">
                                  <el-option label="校级维度" :value="1"></el-option>
                                  <el-option label="非校级维度" :value="2"></el-option>                                  
                                </el-select>
                                <!-- <el-input v-model="scope.row.sortNumber" style="width:80px;" maxlength="10" placeholder="请输入排序值" size="mini"></el-input> -->
                           <!--     <i class="delfont el-icon-check" style="color:#67C23A" @click="saveDimension(scope.row)"></i>
                                <i class="delfont el-icon-close" v-if="scope.row.new" @click="cancel(scope.row)"></i> -->
                            </div>
                            <span v-else>
                                 {{scope.row.dimensionType==1?"校级维度":"非校级维度"}} 
                                 <!-- <i class="dimensionEdit el-icon-edit" @click="editDimension(scope.row)"></i> -->
                            </span>
                        </template>                      
                    </el-table-column>
                    <el-table-column prop="sortNumber" label="排序值" align="center">
                        <template slot-scope="scope">
                           
                            <div class="inline" v-if="scope.row.level==1&&(scope.row.edit)">
                                <el-input v-model="scope.row.sortNumber" style="width:80px;" maxlength="10" placeholder="请输入正整数" size="mini"></el-input>
                                <!-- <i class="delfont el-icon-check" style="color:#67C23A" @click="saveDimension(scope.row)"></i>
                                <i class="delfont el-icon-close" v-if="scope.row.new" @click="cancel(scope.row)"></i> -->
                            </div>
                            <span v-else>
                                 {{scope.row.level==1?scope.row.sortNumber:''}} 
                                 <!-- <i class="dimensionEdit el-icon-edit" @click="editDimension(scope.row)"></i> -->
                            </span>
                        </template>                        
                    </el-table-column>
                    ...
                    <el-table-column label="操作" width="300" align="center">
                        <template slot-scope="scope">
                            ...
                            <el-button type="primary" size="mini" @click="scope.row.edit?saveDimension(scope.row):editDimension(scope.row)" >{{scope.row.edit?'保存':'编辑'}}</el-button>
                            <el-button type="primary" size="mini" v-if="scope.row.new" @click="cancel(scope.row)" >取消</el-button>
                            <!-- <i class="el-icon-afsadf" style="width: 116px;height: 14px;margin: 0 8px;" v-else></i>无意义，占位 -->
                            <!-- <span class="addfont">增加下级维度</span> -->
                            <!-- 没有子维度且不在编辑状态才可以删 -->
                            <el-button  type="danger" icon="el-icon-delete" size="mini" v-if="!scope.row.new" :disabled="!((!scope.row.dimensionItemVOList||scope.row.dimensionItemVOList.length==0)&&!scope.row.edit)" @click="deleteDimension(scope.row)">删除</el-button>
    ...
    methods: {
        ...
        addSubDimension(row){
            ...
            if(row.dimensionItemVOList){
                row.dimensionItemVOList.unshift({
                    ...
                    updateTime:'',
                    name:"",
                    dimensionType:row.dimensionType,//子维度类型与一级维度一致
                    level: row.level+1,
                    pid: row.id,
        ...
        saveDimension(row){
            ...
            if(row.name==''){
              ...
              return;           
            } else if(row.sortNumber=='' && row.level==1){
              this.$notify({
                  type: 'error',
                  message: '请填写排序值！',
                  duration: 1000
            ...
            var reg = /^[1-9]\d*$/;
            if(!reg.test(row.sortNumber) && row.level==1){
              this.$notify({
                  type: 'error',
                  message: '排序值请填写正整数！',
                  duration: 1000
