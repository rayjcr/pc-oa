  ...
  <div class="app-container">
          ...
          <el-form ref="form" :model="form">
            <el-row>
              <el-form-item label="" style="min-width:600px;">
                ...
                <el-button v-if="canEdit" type="primary" @click.stop="showWJSetting">文技分数设置</el-button>
              </el-form-item>
        ...
        <div class="content_right">
          ...
          <transition name="fade">
            <div v-if="!isLoadReport&&curName!=''">
              ...
              <div class="bgdBlock">
                <h4>班主任寄语</h4>
                <el-input resize="none" type="textarea" style="width:500px" placeholder="请输入班主任寄语，最多200字" v-model="curStuInfo.classTeacherMessage"
                  maxlength="200" :rows="10" :disabled="!canEdit" show-word-limit>
                </el-input>
              </div>
            </div>
          </transition>
    ...
    <el-dialog title="项目导入" :visible.sync="xmImportData" width="540px" class="import_box">
      ...
      <el-row style="margin-bottom:10px;">
        ...
        <el-col :span="18">
          <el-select v-model="BatchSelClass" size="small" placeholder="请选择">
            <el-option v-for="(item,index) in backAllStudents" :key="item.bjdm" :label="item.bjmc" :value="item.bjdm"
              :disabled="!item.canEdit"></el-option>
          </el-select>
        </el-col>
      </el-row>
      ...
      <el-row>
        ...
        <el-col :span="18">
          <el-upload :limit="1" name="file" ref="BatchbzForm" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            action="/api/evaluation/report/reportMessageImport" :headers="headersV2" :file-list="bzList" :onError="uploadError"
            :onSuccess="(res,file,fileList)=>{return uploadSuccess(res,file,fileList,'xm')}" :before-upload="beforeUpload"
            :auto-upload="false">
      ...
      <el-row>
        ...
        <el-table :data="errData" style="width: 100%;max-height: 200px;
                            overflow-y: scroll;">
    ...
    <!-- 文技分数设置弹层 -->
    <el-dialog title="文技分数设置" :visible.sync="settingWJScore" class="wjSettingDialog" width="540px">
      <el-alert title="两个规则只能选择一个" type="info" close-text="知道了">
      </el-alert>
      <div class="rule">
        <el-radio v-model="wjRule.rule" :label="1">
          规则一：<br>
          学生端，文、技维度显示分=维度实际得分/<el-input style="width: 50px;" v-model="wjRule.value[0]" :disabled="wjRule.rule!=1"></el-input>
        </el-radio>
        <div class="eg">
          例：“文”维度，学生参与5门课，每门得分90分，如设置规则为维度实际得分/10，则“文”维度显示45分
        </div>
      </div>
      <div class="rule">
        <el-radio v-model="wjRule.rule" :label="2">
          规则二：<br>
          学生端，文、技维度显示分=维度实际得分/维度满分值*<el-input style="width: 80px;" v-model="wjRule.value[1]" :disabled="wjRule.rule!=2"></el-input>
        </el-radio>
        <div class="eg">
          例：“文”维度，学生参与5门课，每门得分90分（每门满分100分），如设置规则为按比例转换，则“文”维度显示（450/500*100）=90分
        </div>
      </div>
      <el-popover placement="left" width="700" offset='100' trigger="hover">
        <img class="showMobile" src="static/quality/showimg.png" alt="">
        <el-button type="text" slot="reference">查看移动端截图</el-button>
      </el-popover>

      <!-- <el-button type="text">查看移动端截图</el-button> -->
      <div slot="footer" class="dialog-footer">
        <el-button size="medium" type="primary" @click="saveConfig">保 存</el-button>
        <el-button size="medium" @click="settingWJScore=false">取 消</el-button>
      </div>
    </el-dialog>
  ...
  export default {
    ...
    data() {
      return {
        ...
        errData: [], //导入错误信息
        honorClass: [], //有权限修改的班级
        settingWJScore: false, //文技分数设置弹框
        wjRule: {
          originRule: 1,
          rule: 1, //文技分数控制规则
          origin: ['10', '100'], //文技分数控制原始值
          value: ['10', '100'], //文技分数控制数值
        },
      }
    },
    computed: {
      ...
      searchName(newname, oldname) {
        if (newname == "") {
          ...
          for (var i = 0; i < this.allStudents.length; i++) {
            this.allStudents[i].students = _.filter(this.allStudents[i].students, function(o) {
              return o.xm.indexOf(newname) != -1;
            });
    ...
    methods: {
      // 获取文技分数设置
      async getScoreRule(){
        var res = await qualityApi.getDictConfig();
        var scoreConvertRule  = res.value.courseScoreConvertRule 
        this.wjRule.rule = scoreConvertRule.ruleType ;
        this.wjRule.originRule = scoreConvertRule.ruleType ;
        this.wjRule.value[Number(this.wjRule.rule)-1] = scoreConvertRule.value;
        this.wjRule.origin[Number(this.wjRule.rule)-1] = scoreConvertRule.value;
        this.wjRule.id = scoreConvertRule.id;
      },
      /**
       * 说明:初始化页面数据 
       * 作者:RayJ
      ...
      async init() {
        //获取当前学年
        var res = await mainProject.dqxnxqObj({
        ...
        this.honorClass = await reportApi.getClassTeacherClassList({
          "schoolCode": localStorage.getItem("xxdm"),
          "teacherCode": localStorage.getItem("usercode")
        })
        // 根据当前班级判断是否可修改
        ...
        if (localStorage.getItem("lguserType") == 2) {
          ...
          for (let i = 0; i < this.allStudents.length; i++) {
            ...
            for (let j = 0; j < this.allStudents[i].students.length; j++) {
              _.assign(this.allStudents[i].students[j], {
                bjnjindex: i
              });
            }
            //合并所有班级的学生到一个变量里
       ...
       */
      async getAllClass() {
        var res = await reportApi.Getallbjlist({
          "xxdm": localStorage.getItem("xxdm"),
          'dqxn': this.curXn
        });
      ...
      async getAllStudent(bjdm, index) {
        var res = await detailBase.getAllStudent({
          "xxdm": localStorage.getItem("xxdm"),
          "xn": this.curXn,
          "bjid": bjdm
        });
      ...
      async getClassDetails(bjList, index) {
        ...
        var res = await qualityApi.getReportMessageClassEntryStatus({
          "classCode": classCode,
      ...
      showWJSetting() {
        this.wjRule.rule = Number(this.wjRule.originRule);
        this.wjRule.value[Number(this.wjRule.rule)-1] = this.wjRule.origin[Number(this.wjRule.rule)-1];        
        this.settingWJScore = true;
      },
      ...
      async batOutputSubmit() {
        ...
        var njdm = _.find(this.backAllStudents, {
      ...
      getPageInfo(uuid) {
        ...
        return _.find(this.resReportAll.page, {
          "uuid": uuid
        })
      },
      ...
      async saveConfig(){
        var params = {
          configType: "courseScoreConvertRule",
          data: {
            enable: true,
            ruleType: this.wjRule.rule,
            value: this.wjRule.value[Number(this.wjRule.rule)-1]
          }
        }
        var reg = /^[0-9]*$/;
        if(!reg.test(params.data.value)){
          this.tipsXX('请输入正整数', 'error', '提示');
          return ;
        }
        var value = Number(params.data.value)
        if(value==0){
          this.tipsXX('请输入正整数', 'error', '提示');
          return ;          
        } else {
        ...
        var res = await qualityApi.saveDictConfig(params);
        if(res.resultCode== 1){
          this.tipsXX('提交成功', 'success', '提示');
          this.settingWJScore = false;
          this.getScoreRule()
        }
      },
