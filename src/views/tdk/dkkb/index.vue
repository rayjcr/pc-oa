...
<script>
  import { getSubstituteWeek, getTeacherSchedule, getClassStructure, queryJsList } from '@/api/tdk/dkkb/index';
  ...
  export default {
    ...
    methods: {
      ...
      exportKb() {
        ...
        jcsj.exportTable2Excel(tableHtml, this.currentLabel + '代课课表', 20);
      },
      // 导出教师所有时间段的课表
      exportAllKb() {
        const that = this;
      ...
      // 刷新课表
      freshTableChage() {
        const kbTime = this.getweek();
        getClassStructure({ xxdm: this.xxdm }).then(timeTableRes => {
          queryJsList({ zgh: this.currentZgh, xxdm: this.xxdm, time: kbTime }).then(res => {
            const jsArr = [];
            const initKb = [];
            this.pkzs = timeTableRes.pkzs.split(',');
            // 格式化数组
