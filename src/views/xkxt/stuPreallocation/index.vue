<template>
  <section class="content-container app-container stupreallocation">
    ...
    <el-col :span="24" class="toolbar">
        ...
        <el-table :key='tableKey' :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
            ...
            <el-pagination class="pagination" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="listQuery.page" :page-sizes="[10,20,30, 50]" :page-size="listQuery.limit" layout="total, sizes, prev, pager, next, jumper" :total="total"> </el-pagination>
    </div>
    </div>
      <!--导入界面-->
      <el-dialog title="导入数据" :visible.sync="dialog_1" width="560px">
          ...
          <el-row>
              <el-button size="small" type="primary" @click="download">模板下载</el-button>
          </el-row>
          <el-row>
        ...
        <el-dialog title="导入数据" :visible.sync="dialog_2" @close="cancel_dr">
          ...
          <el-table :key='tableKey' :data="errlist" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
              ...
              <el-table-column align="center" label="学生姓名">
                  <template slot-scope="scope">
                    <span>{{scope.row.zd1}}</span>
                  </template>
              </el-table-column>
              <el-table-column align="center" label="班级">
                  <template slot-scope="scope">
                    <span>{{scope.row.zd2}}</span>
                  </template>
              </el-table-column>
      ...
      <!--新增界面-->
	    ...
	    <el-dialog title="分配" class="fpDialog" :visible.sync="dialogFormVisible" @close="colseDialog" width="900px">
          <el-form label-width="80px" ref="addForm">
            <!--已选学生-->
              <el-collapse style="margin-top: 5px" v-model="activeName" accordion>
                <el-collapse-item name="1">
                  <template slot="title">
                    <i class="header-icon el-icon-user-solid"></i>已选学生：
                  </template>
                  <div>
                    <el-tag
                      size="small"
            ...
            <el-row>
             <!-- <span style="padding-right:10px;">筛选条件:</span>-->
              <!-- <el-button type="primary" @click="bjsubmit" >按班级</el-button>
               <el-button type="primary" @click="tcssubmit">按特长生类型</el-button>-->
              <el-button-group style="margin-top: 5px">
                <el-button type="primary" icon="el-icon-postcard" @click="bjsubmit" :plain="plainButton[0]">按班级</el-button>
                <el-button type="primary" icon="el-icon-s-check" @click="tcssubmit" :plain="plainButton[1]">按特长生类型</el-button>
              </el-button-group>
            </el-row>
                ...
                <el-row :gutter="12">
                  <el-col :span="8">
                    <el-card shadow="never">
                      <!--年级班级展示-->
                      <el-tree :data="treeData" show-checkbox node-key="id" check-strictly accordion ref="tree" :highlight-current="true" :default-expanded-keys="defaultExpanded" @node-click="handleCheckClick" @check-change="handleCheckChange">
                  ...
                  <el-col :span="16">
                    <el-card shadow="hover" class="cardList">
                      <!--班级学生展示-->
                      <div class="noStu" v-if="studentArry.length==0">暂无数据请勾选班级</div>
        ...
        <span slot="footer" class="dialog-footer">
          <el-button @click="dialogFormVisible = false">取 消</el-button>
           <el-button type="primary" icon="el-icon-folder-add" @click="bc">保存</el-button>
        </span>
	    </el-dialog>
...
import { dqxnxqObj, page, xsyfpxq, getIstsdkc, getBjSduntByBjid, getTcsXs, saveXsxkInfo, exportXsyfpMb, againUp } from 'api/xkxt/stuPreallocation/index';
export default {
  ...
  data() {
    return {
      ...
      dialog_2: false,
      errlist: [],
      importUrl: '/api/course/xkxtXsyfp/importExcel',
      importHeaders: {
  ...
  methods: {
    getList() {
      ...
      const cxlx = this.listQuery.cxlx;
      if (cxlx == '') {
    ...
    handleInputConfirm(v) { /** @班级学生选中增加已选学生方法**/
      if (v.xh) {
        // 判断学号是否已被选
        const curxh = this.yfpXsList.find(item=>item.xh==v.xh);
        console.log(curxh)
        if(!curxh){
          this.yfpXsList.push(v);
        }
      }
    ...
    download() {
      ...
      exportXsyfpMb().then((res)=>{
        ...
        downloadElement.download = '学生预分配.xls'; // 下载后文件名
...
<style lang='scss' scoped>
    ...
    .app-container .el-checkbox{ margin-right: 20px;}
    ...
    .fpDialog /deep/ .el-card__body{
      padding: 10px;
    }
    .fpDialog /deep/ .el-tree-node__expand-icon.is-leaf{
      display: none;
    }
    .cardList{
      min-height: 250px;
    ...
    .fpDialog .app-container .el-checkbox{
      margin-right: 0;
    }
</style>
