<template>
    <div class="calendar-list-container">
      ...
      <el-col :span="24" class="toolbar">
        <el-form :inline="true">
          <el-form-item>
            <el-button type="info" plain @click="handleExportXsmd">按班级导出选课名单</el-button>
            <el-button type="info" plain @click="handleExportXsmx">导出学生选课结果</el-button>
...
<script>
import { xsxxPage, ckxkjg, getXkmdUrl, sendXktz, getAllXsgx, tcsPage,exportXsxkmd,exportXsxkmx} from 'api/xkxt/xkjgmanager/index';
...
export default {
  ...
  methods: {
    ...
    // 按班级导出学生选课名单
    async handleExportXsmd() {
      this.exportLoading = true;
      const data = this.$refs.searchbar.getParam();
      var param = {};
      param.ssxn = this.dqxn;
      param.xxdm = this.xxdm;
      param.ssxq = this.dqxq;
      param.cxmc = data.curValue;
      param.njGroup = data.temp[0]+'';
      param.bjGroup = data.temp[1]+'';
      param.xsXnGroup = data.temp[2]+'';
      param.xsXqGroup = data.temp[3]+'';
      if (data.temp.length < 5 || data.temp[4].length === 0) {
        param.xkztGroup = '';
      } else {
        param.xkztGroup = data.temp[4]+'';
      }
      var res = await exportXsxkmd(param);
      ...
      downloadElement.download = '学生选课名单.xls'; // 下载后文件名
    ...
    async handleExportXsmx() {
      ...
      var res = await exportXsxkmx(param);
      const blob = new Blob([res], {
        type:
          'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8'
      });
