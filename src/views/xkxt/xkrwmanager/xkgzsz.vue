...
<div class="app-container calendar-list-container">
  ...
  <div class="oarow u_TableMain">
    <div class="filter-container innerTab">
      <el-button v-if="isEdit" type="primary" @click="saveGz('form')">保存</el-button>
    ...
    <div class="m_GenTableBox">
      <el-alert
        ...
        title=" 温馨提示：当有选课任务正在进行中时，不可修改选课规则!!"
        type="warning"  show-icon>
      </el-alert>
      <el-col>
        <el-card shadow="hover">
          <div class="rowmain">
            <el-form :model="form" ref="form" :rules="formrules" label-width="180px" name="myform">
              <el-row>
              <el-form-item label="学年：">
                <el-select v-model="dqxn" @change="showRule"
                           placeholder="请选择学年">
                ...
                学期：
                <el-select v-model="dqxq" @change="showRule"
                           placeholder="请选择学期">
                ...
                <el-form-item  label="不能同时选择的课程"  >
                  <div class="">
                    <div class="rowRightBox">
                      <el-button v-if="isEdit" type="primary" icon="el-icon-plus" size="small" plain @click="addhcKc" >添加</el-button>
                      <div class="courseList">
                        <div class="course"  v-for="hckc in hckclist" :label="hckc.kcdms" :key="hckc.zhid">
                          <span>{{hckc.kcmcs}}</span> <i v-if="isEdit" class="el-icon-edit" @click="edithckc(hckc.zhid,hckc.kcdms)"></i> <i v-if="isEdit" class="el-icon-delete" @click="delhckc(hckc.zhid)"></i>
                        </div>
                      </div>
                      <div v-if="!isEdit && hckclist.length == 0">暂无</div>
                    </div>
                  </div>
                </el-form-item>
              </el-row>
              <el-row>
                <el-form-item  label="必选的课程类别"  >
                  <div class="">
                    <div class="rowRightBox">
                      <el-button v-if="isEdit" type="primary" icon="el-icon-plus" size="small" plain @click="addbxKc">添加</el-button>
                      <div class="courseList">
                        <el-form-item prop="zdxks">
                          <div class="course" v-for="(lbkc, index) in lbkclist" :label="lbkc.lbid" :key="index">
                            <span>{{lbkc.lbmc}} 至少选 <el-input style="display:inline-block; width:50px;" placeholder="" v-model.trim="lbkclist[index].zdxks" ></el-input> 门课</span> <i v-if="isEdit" class="el-icon-delete" @click="delBxkc(lbkc.uuid)"></i>
                          </div>
                        </el-form-item>

                      </div>
                      <div v-if="!isEdit && lbkclist.length == 0">暂无</div>
                    </div>
                  </div>
                </el-form-item>
              </el-row>
        ...
        <el-row>
          ...
          <div class="courseCheckList">
            <el-collapse style="width: 100%;">
              <div v-for="(value,key) in xkkclist" :key="key">
                <el-collapse-item>
                  <template slot="title">
                    {{key}}
                  </template>
                  <el-checkbox-group v-model="checkkcxx">
                    <el-checkbox v-for="kcxx in value" :label="kcxx.kcdm" :key="kcxx.kcdm">{{kcxx.kcmc}}</el-checkbox>
                  </el-checkbox-group>
                </el-collapse-item>
              </div>
            </el-collapse>
...
<script>
  import { getxnList,getxqList,dqxnxqObj } from 'api/components/searchbar'
  import { addGzszHckc, getHckcList, deleteHckc, getGzszXklb, addGzszBxkc, getBxkcList, saveGzsz, deleteBxkc, getXktjzdks ,getCurrentXkrw,getKclxKcxx} from 'api/xkxt/xkrwmanager/index'
  export default {
    name: 'xkgzsz',
    ...
    created() {
      ...
      dqxnxqObj({ xxdm: this.xxdm }).then(response => {
        ...
        this.xndm = response.data.xn;
        this.xqdm = response.data.xq;
    ...
    methods: {
      showRule(){
        let flag = false;
        if(this.dqxn==this.xndm&&this.dqxq==this.xqdm){
          flag= true;
        }
        //查询是否有正在进行的选课任务
        getCurrentXkrw().then(res=>{
          if(res.data>0){
            flag = false;
          }
          this.isEdit = flag;
