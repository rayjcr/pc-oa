    ...
    <section class="app-container content-container">
                ...
                <el-row v-if="list.length > 0" :gutter="12" style="margin-bottom:6px;" v-for="(item,index) in list" :key="index">
                    ...
                    <el-card shadow="always" :class="classColor(item.pkzt)">
                        ...
                        <el-row style="float:right;">
                            <el-button @click="handleUpdate(item)" size="mini" type="primary" icon="el-icon-edit-outline">编辑
                            </el-button>
                            <el-button @click="openData(item)" size="mini" type="success" icon="el-icon-tickets" :disabled="listRwZt[item.pkzt].qy">
                            {{item.pkzt==2?'禁用':'启用'}}
                            </el-button>
                            <el-button @click="deleteData(item.jlid)" size="mini" type="danger" icon="el-icon-delete"
                                    :disabled="item.pkzt==2?true:false">删除
                            </el-button>
                        </el-row>
                        </div>
                        <p></p>
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogForm" width="29%" @close="colseDialog">
            ...
            <el-form-item label="学期" prop="ksxq">
                ...
                <el-option v-for="(item,index) in  xqList" :key="(index+1)" :label="item.xqmc"
    ...
    data() {
      ...
      return {
        ...
        listRwZt: [/** 列表显示的三种状态**/
          ...
          { ztmc: '已启用', pksz: true, pktz: true, pkjg: false, qy: false, lspk: false }
        ]
      }
    },
    filters: {
    ...
    methods: {
      ...
      openData(e) {
        ...
        this.$confirm(messge, '提示', { type: 'warning' }).then(() => {
          const data = { jlid: e.jlid, pkzt: rwzt }
          const tempData = Object.assign({}, data);
          api.getUpdate(e.jlid, tempData).then(() => {
