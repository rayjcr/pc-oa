...
<script>
  import { kczdpk, getStudenNumberByAuto } from '@/api/znpk/examination/znpk/index';
  ...
  export default {
    ...
    methods: {
      async autopk() {
        await kczdpk(this.ksid).then(res => {
          this.step = 'step2';
          this.checkStep();
      ...
      getSckh() {
        getStudenNumberByAuto({ examId: this.ksid }).then(res => {
          if (parseInt(res.status) === 200) {
          } else {
            this.$notify({ title: '失败', message: res.message, type: 'error', duration: 2000 });
          }
